...
const transactionStates = new WeakMap<
  ...
  { depth: number; nextSavepointId: number }
>();

function getTransactionState(sqlite: NativeDatabase.Database): {
  depth: number;
...
function createTransaction(sqlite: NativeDatabase.Database): SqliteTransaction {
  return function transaction<Args extends unknown[], Result>(
    fn: (...args: Args) => Result,
  ) {
...
function installSqliteCompat(
  ...
  Object.defineProperty(compat, "exec", {
    configurable: true,
    value: (sql: string) => {
...
function resolveMigrationsDir(): string {
  ...
  if (!found)
    throw new Error("Runtime migration assets are missing from this build.");
  return found;
}
...
function bootstrapMigrationLedger(
  sqlite: NativeDatabase.Database,
  files: string[],
): void {
  const row = sqlite
  ...
  const legacy = sqlite
    .prepare(
      "SELECT name FROM sqlite_master WHERE type = 'table' AND name IN ('wiki_snapshots', '_meta')",
    )
    .all() as Array<{ name: string }>;
  if (legacy.length === 0) return;
  const runtimeTables = sqlite
    .prepare(
      "SELECT name FROM sqlite_master WHERE type='table' AND name IN ('agent_runtime_sessions', 'agent_runtime_runs')",
    )
    .all() as Array<{ name: string }>;
  if (runtimeTables.length !== 2)
    throw new Error(
      "Legacy database predates the supported runtime baseline. Back up and migrate it with the matching Synax version; no migration ledger was guessed.",
    );
  ...
  pinoLogger.info(
    ...
    "context db: bootstrapped migration ledger for existing database",
  );
}
...
function runMigrations(sqlite: NativeDatabase.Database): void {
  ...
  for (const f of files) {
    if (isMigrationApplied(sqlite, f)) {
      pinoLogger.info(
        { file: f },
        "context db: migration skipped (already applied)",
      );
      continue;
    }
...
function migrateWikiEvaluationsToGoals(sqlite: NativeDatabase.Database): void {
  const legacy = sqlite
    .prepare(
      "SELECT name FROM sqlite_master WHERE type = 'table' AND name = 'wiki_evaluations'",
    )
    .get() as { name: string } | undefined;
  if (!legacy) return;

  const columns = sqlite
    .prepare("PRAGMA table_info(wiki_evaluations)")
    .all() as Array<{ name: string }>;
...
function ensureRuntimeSchema(sqlite: NativeDatabase.Database): void {
  ...
  ensureColumn(
    ...
    "agent_runtime_tool_calls",
    "mutability",
    "mutability TEXT NOT NULL DEFAULT 'read'",
  );
  ensureColumn(
    sqlite,
    "agent_runtime_tool_calls",
  ...
  try {
    sqlite.exec(
      `UPDATE wiki_plans SET goal_ids_json = evaluation_ids_json WHERE goal_ids_json = '[]' AND evaluation_ids_json != '[]'`,
    );
    sqlite.exec(
      `UPDATE wiki_plan_nodes SET goal_ids_json = evaluation_ids_json WHERE goal_ids_json = '[]' AND evaluation_ids_json != '[]'`,
    );
  } catch {
    /* columns may not exist on fresh DB */
  }
}
...
function getOrCreateRawSqlite(dbPath = resolveDbPath()): RawSqlite {
  ...
  try {
    ...
    if (shouldRunMigrationsInThisProcess()) installConversationHistoryTriggers(sqlite);
  } catch (error) {
    sqlite.close();
    throw error;
  }
...
function getOrCreateClient(dbPath = resolveDbPath()): Client {
  ...
  client.execute = ((statement: InStatement, args?: unknown) => {
    ...
    return (execute as (...input: unknown[]) => ReturnType<Client["execute"]>)(
      statement,
      args,
    );
  }) as Client["execute"];
  ...
  client.transaction = (async (mode?: TransactionMode) => {
    ...
    tx.execute = ((statement: InStatement, args?: unknown) => {
      ...
      if (/\b(?:INSERT|UPDATE|DELETE|REPLACE|CREATE|DROP|ALTER)\b/i.test(sql))
        assertDatabaseWriteAllowed(getOrCreateRawSqlite(dbPath), dbPath);
      return (
        executeTx as (...input: unknown[]) => ReturnType<Client["execute"]>
      )(statement, args);
...
export function assertRuntimeExecutionCurrent(): void {
  assertDatabaseWriteAllowed(getRawSqlite(), resolveDbPath());
}
...
export function tryGetRawSqlite(): RawSqlite | null {
  return _sqlite;
}
