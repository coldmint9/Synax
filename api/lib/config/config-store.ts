...
function getGlobalConfigInternal(includeSecrets: boolean): GlobalConfig {
  ...
  return mergeGlobalConfigLayers(
    layers.template,
    layers.global,
    includeSecrets,
  );
...
export function updateGlobalConfig(
  ...
  writeJsonAtomic(
    templateConfigPath(),
    prepareGlobalConfigForStorage(nextLayers.template),
  );
  writeJsonAtomic(
    globalConfigPath(),
    prepareGlobalConfigForStorage(nextLayers.global),
  );
...
export function getProjectConfigForRuntime(
  projectId: string,
): ProjectConfig | null {
  return getProjectConfigInternal(projectId, true);
}

function getProjectConfigInternal(
  projectId: string,
...
export function getEffectiveConfigForDisplay(
  projectId: string,
): EffectiveConfig {
  return getEffectiveConfigInternal(projectId, false);
}

function getEffectiveConfigInternal(
  projectId: string,
  includeSecrets: boolean,
): EffectiveConfig {
  ...
  const globalConnection = global.providerConnections[providerId] ?? {
    providerId,
  };
...
export function listAvailableProviders(): ProviderDef[] {
  return getGlobalConfig().providers.filter(
    (provider) => provider.status !== "inactive",
  );
}

export function getProviderDetail(providerId: string): ProviderDef | undefined {
  return getGlobalConfig().providers.find(
    (provider) => provider.id === providerId,
  );
}

export function getAnalyzerLlmConfig(
  includeSecrets = true,
): AnalyzerLlmConfig | null {
  const global = getGlobalConfigInternal(includeSecrets);
  const provider = global.providers.find(
    (item) => item.id === global.defaultApiProviderId,
  );
    ...
    if (!templateExists && globalExists) {
      ...
      if (!migrated) {
        if (!templateExists) {
          writeJsonAtomic(
            templateConfigPath(),
            prepareGlobalConfigForStorage(createDefaultGlobalConfig()),
        ...
        if (!globalExists) {
          writeJsonAtomic(
            globalConfigPath(),
            prepareGlobalConfigForStorage(createDefaultUserGlobalConfig()),
            { preserveBackup: false },
          );
        }
      }
...
function migrateLegacyJsonConfigIfNeeded(): boolean {
  ...
  writeJsonAtomic(
    templateConfigPath(),
    prepareGlobalConfigForStorage(split.template),
    { preserveBackup: false },
  );
  writeJsonAtomic(
    globalConfigPath(),
    prepareGlobalConfigForStorage(split.global),
    { preserveBackup: false },
  );
  return true;
}
...
function migrateLegacyConfigIfNeeded(): boolean {
  ...
  const globalRow = db
    .prepare(
      "SELECT config_json, version, updated_at, updated_by FROM global_config WHERE id = 1",
    )
    .get() as
    | {
        config_json: string;
  ...
  const projectRows = db
    .prepare(
      "SELECT project_id, config_json, version, updated_at, updated_by FROM project_config",
    )
    .all() as Array<{
    project_id: string;
    config_json: string;
  ...
  for (const row of projectRows) {
    ...
    writeJsonAtomic(
      projectConfigPath(row.project_id),
      prepareProjectConfigForStorage(projectConfig),
      { preserveBackup: false },
    );
  }
  ...
  logger.info(
    ...
    "[config] legacy sqlite config migrated to json",
  );
  return true;
}
...
function splitMergedGlobalConfig(config: GlobalConfig): ConfigLayers {
  ...
  const globalBase = createDefaultUserGlobalConfig(
  ...
  return {
    template: {
      ...
      providerConnections: mergeProviderConnections(
        templateBase.providerConnections,
        templateConnections,
        true,
        templateBase.providers,
      ),
...
function normalizeMergedGlobalConfig(
  config: GlobalConfig,
  includeSecrets: boolean,
): GlobalConfig {
...
function normalizeTemplateConfig(
  ...
  const incomingProviders = (config.providers ?? []).filter((provider) =>
    isTemplateProviderId(provider.id),
  );
  const providers = mergeProviderDefs(defaults.providers, incomingProviders);
  const incomingConnections = pickConnections(
    config.providerConnections ?? {},
    TEMPLATE_PROVIDER_IDS,
  );
  const providerConnections = mergeProviderConnections(
  ...
  return {
    ...
    providerConnections,
    defaultProviderId: normalizeAcpProviderId(config.defaultProviderId),
    defaultApiProviderId: normalizeApiProviderId(
      config.defaultApiProviderId,
      providers,
    ),
    enabledAcpProviderIds:
...
function normalizeUserGlobalConfig(
  config: GlobalConfig,
  includeSecrets: boolean,
): GlobalConfig {
  const defaults = createDefaultUserGlobalConfig(config.updatedBy ?? "system");
  const incomingProviders = (config.providers ?? []).filter(
    (provider) => !isTemplateProviderId(provider.id),
  );
  ...
  const providerConnections = normalizeConnections(
    incomingConnections,
    includeSecrets,
  );
  return {
    ...
    defaultProviderId: normalizeAcpProviderId(config.defaultProviderId),
    defaultApiProviderId:
      config.defaultApiProviderId || defaults.defaultApiProviderId,
    enabledAcpProviderIds:
      config.enabledAcpProviderIds ?? defaults.enabledAcpProviderIds,
    mcpServers: normalizeMcpServers(
      config.mcpServers ?? defaults.mcpServers ?? [],
...
function mergeGlobalConfigLayers(
  template: GlobalConfig,
  global: GlobalConfig,
  includeSecrets: boolean,
): GlobalConfig {
  ...
  const providerConnections = mergeProviderConnections(
    template.providerConnections,
    global.providerConnections,
    includeSecrets,
    providers,
  );
  ...
  return {
    ...
    defaultApiProviderId: normalizeApiProviderId(
      global.defaultApiProviderId ?? template.defaultApiProviderId,
      providers,
    ),
    enabledAcpProviderIds:
      global.enabledAcpProviderIds ?? template.enabledAcpProviderIds,
    providerConnections,
    mcpServers: normalizeMcpServers(
...
function applyGlobalConfigPatch(
  ...
  const templatePatchProviders = patch.providers?.filter((provider) =>
    isTemplateProviderId(provider.id),
  );
  const userPatchProviders = patch.providers?.filter(
    (provider) => !isTemplateProviderId(provider.id),
  );
  const templatePatchConnections = pickConnections(
    patch.providerConnections ?? {},
    TEMPLATE_PROVIDER_IDS,
  );
  const userProviderIds = new Set(
    (userPatchProviders?.length
      ? userPatchProviders
      : layers.global.providers
    ).map((provider) => provider.id),
  );
  const userPatchConnections = pickConnections(
    patch.providerConnections ?? {},
    userProviderIds,
  );
  ...
  const nextTemplateConnections =
    templatePatchProviders || Object.keys(templatePatchConnections).length > 0
      ? mergeProviderConnections(
          ...
          templatePatchConnections,
          true,
          nextTemplateProviders,
        )
      : layers.template.providerConnections;

  const nextGlobalProviders = userPatchProviders
    ? normalizeProviders(userPatchProviders)
    : layers.global.providers;
  const nextGlobalConnections =
    userPatchProviders || Object.keys(userPatchConnections).length > 0
      ? mergeProviderConnections(
          layers.global.providerConnections,
          userPatchConnections,
          true,
          nextGlobalProviders,
        )
      : layers.global.providerConnections;
  ...
  const templateTouched =
    Boolean(templatePatchProviders?.length) ||
    Object.keys(templatePatchConnections).length > 0;
  ...
  const nextTemplate: GlobalConfig = {
    ...
    version: templateTouched
      ? layers.template.version + 1
      : layers.template.version,
  };

  const allProviders = mergeProviderDefs(
    nextTemplateProviders,
    nextGlobalProviders,
  );
  ...
  const nextGlobal: GlobalConfig = {
    ...
    defaultProviderId: normalizeAcpProviderId(
      patch.defaultProviderId ?? current.defaultProviderId,
    ),
...
function normalizeConnections(
  ...
  for (const [providerId, connection] of Object.entries(connections)) {
    result[providerId] = normalizeProviderConnection(
      connection,
      includeSecrets,
    );
  }
...
function normalizeProviderConnection(
  ...
  const apiKeyMasked =
    connection.apiKeyMasked ??
    (includeSecrets || !isEncryptedSecret(connection.apiKey)
...
function prepareProjectConfigForStorage(config: ProjectConfig): ProjectConfig {
  return {
    ...config,
    providerConnection: config.providerConnection
      ? prepareProviderConnectionForStorage(
          config.providerConnection,
          undefined,
        )
      : config.providerConnection,
...
function prepareProviderConnectionsForStorage(
  connections: Record<string, ProviderConnection>,
): Record<string, ProviderConnection> {
...
function normalizeApiProviderId(
  ...
  providers: ProviderDef[] = BUILTIN_PROVIDERS,
): string {
...
function isBuiltinApiProviderId(
  providerId: string,
): providerId is "openai" | "anthropic" {
  return providerId === "openai" || providerId === "anthropic";
}

function isCustomApiProviderId(providerId: string): boolean {
  return (
    providerId.startsWith(CUSTOM_API_PROVIDER_PREFIX) &&
    providerId.length > CUSTOM_API_PROVIDER_PREFIX.length
  );
}

function isApiProviderId(providerId: string): boolean {
  return (
    isBuiltinApiProviderId(providerId) || isCustomApiProviderId(providerId)
  );
}
...
function readJsonFile<T>(filePath: string): T | null {
  ...
  if (!fs.existsSync(filePath)) {
    if (fs.existsSync(backupPath) && isJsonFileValid(backupPath)) {
      ...
      logger.warn(
        { filePath },
        "[config] restored missing json file from backup",
      );
      return parsed;
    }
    return null;
  ...
  try {
    ...
    logger.error(
      ...
      "[config] failed to read json file",
    );
    throw err;
  }
}
...
function writeJsonAtomic(
  ...
  try {
    ...
    logger.error(
      { filePath, err: message },
      "[config] failed to write json file",
    );
    throw err;
  }
}
