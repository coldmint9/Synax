...
const globalConfigPatchSchema = z
  .object({
    ...
    providerConnections: z
      .record(z.string(), providerConnectionSchema)
      .optional(),
    mcpServers: z
      .array(
        z
...
configRoutes.put("/global", async (c) => {
  ...
  const updatedBy =
    c.req.header("X-User-Name") ?? c.req.header("X-User-Id") ?? "api";
...
configRoutes.post("/ai-api/models/discover", async (c) => {
  ...
  try {
    ...
    return c.json(
      { ok: false, models: [], source: "api/models", error: message },
      400,
    );
  }
});
...
configRoutes.put("/projects/:projectId/config", async (c) => {
  ...
  try {
    ...
    logger.error(
      ...
      "[config] project config update failed",
    );
    return c.json({ error: message }, 500);
  }
});
...
configRoutes.get("/projects/:projectId/config/effective", (c) => {
  ...
  try {
    ...
    logger.error(
      { err: message, projectId },
      "[config] effective config failed",
    );
    return c.json({ error: message }, 500);
  }
});
...
configRoutes.post("/open-file", async (c) => {
  ...
  const { filePath, line, location } = body as {
    filePath: string;
    line?: number;
...
function validateGlobalConfigPatch(body: unknown): UpdateGlobalConfigRequest {
  ...
  if (!parsed.success) {
    ...
    throw new Error(
      first
        ? `配置参数无效: ${first.path.join(".") || "body"} ${first.message}`
        : "配置参数无效",
    );
  }
  ...
  for (const [providerId, connection] of Object.entries(
    ...
    validateProviderConnection(
      providerId,
      connection,
      current.providerConnections[providerId],
    );
  }

  const defaultAcpProviderId =
...
function validateProviderConnection(
  ...
  if (extra.defaultReasoningEffort !== undefined) {
    ...
    if (
      ...
      throw new Error(
        `${providerId} 的 defaultReasoningEffort 只能是 low/medium/high/xhigh/max`,
      );
    }
  }
...
function isAcpProviderId(
  providerId: string,
): providerId is (typeof ACP_PROVIDER_IDS)[number] {
  return (ACP_PROVIDER_IDS as readonly string[]).includes(providerId);
}

function isBuiltinApiProviderId(
  providerId: string,
): providerId is (typeof BUILTIN_API_PROVIDER_IDS)[number] {
...
async function validateAiApi(
  input: z.infer<typeof aiApiValidateSchema>,
): Promise<{
  ok: boolean;
  message?: string;
  error?: string;
  ...
  try {
    const result = await tryValidateOnce(
      baseUrl,
      input.format,
      apiKey,
      input.model,
...
async function discoverAiApiModels(
  ...
  try {
    const result = await tryDiscoverModels(
      baseUrl,
      input.format,
      apiKey,
      controller.signal,
    ...
    if (result.status === 404 && !baseUrl.endsWith("/v1")) {
      const altUrl = `${baseUrl}/v1`;
      const retry = await tryDiscoverModels(
        altUrl,
        input.format,
        apiKey,
        controller.signal,
...
function extractModelIds(payload: unknown): string[] {
  ...
  return Array.from(
    new Set(candidates.filter((item) => item.trim().length > 0)),
  );
}
...
async function validationError(resp: Response): Promise<string> {
  ...
  if (resp.status === 401 || resp.status === 403)
    return `认证失败 (${resp.status})`;
  if (resp.status === 404) return `接口路径不存在 (${resp.status})`;
  if (resp.status === 400)
    return `请求被拒绝 (${resp.status}): ${text.slice(0, 240)}`;
