...
export async function resolveBrowseDirectory(input?: string): Promise<string> {
  ...
  if (!isAbsolute(raw))
    throw new DirectoryBrowseError("Directory path must be absolute.", 400);
  ...
  try {
    ...
    if (code === "ENOENT")
      throw new DirectoryBrowseError(`Directory not found: ${candidate}`, 404);
    if (code === "EACCES" || code === "EPERM")
      throw new DirectoryBrowseError(
        `Directory is not readable: ${candidate}`,
        403,
      );
    throw new DirectoryBrowseError(
      `Directory is unavailable: ${candidate}`,
      400,
    );
  }
  const info = await stat(canonical);
  if (!info.isDirectory())
    throw new DirectoryBrowseError("Path is not a directory.", 400);
  return canonical;
}
...
async function shortcutsFor(current: string): Promise<string[]> {
  const candidates =
    process.platform === "darwin"
      ? [homedir(), dirname(homedir()), "/Volumes", "/tmp"]
      : [homedir(), dirname(homedir()), "/tmp"];
...
export async function listDirectories(
  ...
  try {
    ...
    if (code === "EACCES" || code === "EPERM")
      throw new DirectoryBrowseError(
        `Directory is not readable: ${canonical}`,
        403,
      );
    throw new DirectoryBrowseError(
      `Directory is unavailable: ${canonical}`,
      400,
    );
  }
  ...
  const visible = dirents
    ...
    .filter(
      (name) =>
        (showHidden || !HIDDEN_ENTRY.test(name)) &&
        (showIgnored || !IGNORED_DIRECTORIES.has(name)),
    )
    .sort((a, b) =>
  ...
  const entries = await filterDirectories(
    canonical,
    truncated ? visible.slice(0, MAX_ENTRIES) : visible,
  );
...
export async function listWslDirectories(
  ...
  const canonical = await canonicalizeWslPath(
    distribution,
    input?.trim() || home,
  );
    ...
    await Promise.all(
      names.map(async (name) => {
        ...
        return {
          name,
          path: linuxPath,
          hidden: HIDDEN_ENTRY.test(name),
        } satisfies DirectoryEntry;
      }),
    )
  ).filter((entry): entry is DirectoryEntry => entry !== null);
  const shortcutCandidates = [home, "/", "/mnt"].filter(
    (candidate) => candidate !== canonical,
  );
  const shortcuts = (
    await Promise.all(
      shortcutCandidates.map(async (candidate) => {
        try {
          return (
            await stat(
              workspaceLocationHostPath({
...
fsRoutes.get("/list", async (c) => {
  ...
  try {
    ...
    const listing =
      parsed.data.locationKind === "wsl"
        ? await listWslDirectories(
    ...
    if (error instanceof WslError) {
      return c.json(
        ...
        error.code === "WSL_DISTRIBUTION_MISSING" ? 404 : 400,
      );
    }
    if (error instanceof DirectoryBrowseError) {
      return c.json(
        { error: error.message, code: "DIRECTORY_UNAVAILABLE" },
        error.status,
      );
    }
    logger.error(
      { err: error instanceof Error ? error.message : String(error) },
      "[fs] directory listing failed",
    );
