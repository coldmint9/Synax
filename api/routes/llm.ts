...
import { saveGeneratedMedia } from "../services/agent-runtime/generated-media.js";
import {
  modelContentParts,
  validateAssets,
...
} from "../services/llm-runtime/gateway.js";
...
llmRoutes.post("/stream", async (c) => {
  ...
  } as import("../services/llm-runtime/types.js").LlmGatewayRequest;
  try {
    input.messages = parsed.data.messages.map((message) => {
      if (!message.contentParts)
        return { role: message.role, content: message.content };
      if (message.role !== "user" || !input.projectId)
        throw new Error("Media requires a user message and projectId.");
      validateAssets(message.contentParts, input.projectId);
      return {
        role: "user" as const,
        content: modelContentParts(message.contentParts),
      };
    });
  } catch (error) {
    return c.json(
  ...
  try {
    ...
    if (err instanceof AgentProviderNotConfiguredError) {
      logger.warn(
        ...
        "[llm] LLM provider not configured for stream",
      );
  ...
  logger.info(
    { projectId: input.projectId, purpose: input.purpose, model: input.model },
    "[llm] stream request",
  );
  ...
  return streamSSE(c, async (stream) => {
    ...
    try {
      ...
      const message = err instanceof Error ? err.message : String(err);
      logger.error(
        { err: message, projectId: input.projectId, purpose: input.purpose },
        "[llm] stream failed",
      );
