...
} from "../services/integrations/local-discovery.js";
import { discoverLocalMcpServers } from "../services/mcp/mcp-discovery.js";

export const projectSettingsRoutes = new Hono();

function projectDiscoveryPath(
  projectId: string,
): { root: string; location: WorkspaceLocation } | null {
  ...
  const location: WorkspaceLocation | undefined =
    source?.kind === "wsl" && source.distribution && source.path
      ...
      : source?.localPath
        ...
        : undefined;
  return location
    ? { root: workspaceLocationHostPath(location), location }
    : null;
}
...
projectSettingsRoutes.get("/:projectId/settings/effective", (c) => {
  ...
  try {
    ...
    logger.error(
      { projectId, err: msg },
      "[project-settings] get effective failed",
    );
    return c.json({ error: msg }, 500);
  }
});
...
projectSettingsRoutes.get("/:projectId/settings/mcp/discovery", (c) => {
  ...
  try {
    ...
    logger.error(
      { projectId, err: msg },
      "[project-settings] MCP discovery failed",
    );
    return c.json({ error: msg }, 500);
  }
});
...
const discoveryInput = z.object({
  directories: z.array(z.string().min(1).max(4096)).max(8).default([]),
});

projectSettingsRoutes.post(
  "/:projectId/settings/integrations/discovery",
  async (c) => {
    const parsed = discoveryInput.safeParse(
      await c.req.json().catch(() => ({})),
    );
    if (!parsed.success)
      return c.json({ error: "Invalid discovery directories" }, 400);
    const workspace = projectDiscoveryPath(c.req.param("projectId"));
    if (!workspace)
      return c.json({ error: "A local project directory is required" }, 404);
    const { paths: _paths, ...result } = discoverLocalIntegrations(
      workspace.root,
      {
        extraDirectories: discoveryDirectories(
          workspace.location,
...
projectSettingsRoutes.post(
  "/:projectId/settings/integrations/skills/import",
  async (c) => {
    const parsed = discoveryInput
      .extend({ id: z.string().regex(/^[a-f0-9]{24}$/) })
    ...
    try {
      return c.json(
        importDiscoveredSkill(workspace.root, parsed.data.id, {
          extraDirectories: discoveryDirectories(
            workspace.location,
            parsed.data.directories,
          ),
...
projectSettingsRoutes.put("/:projectId/settings", async (c) => {
  ...
  try {
    const settings = updateProjectSettings(
      projectId,
      body as any,
      "current-user",
    );
...
projectSettingsRoutes.patch("/:projectId/settings/:section", async (c) => {
  ...
  try {
    const settings = patchProjectSettingsSection(
      projectId,
      parsed.data,
      body,
      "current-user",
    ...
    const msg = err instanceof Error ? err.message : String(err);
    logger.error(
      { projectId, section, err: msg },
      "[project-settings] patch section failed",
    );
    return c.json({ error: msg }, 500);
  }
});
...
projectSettingsRoutes.post("/:projectId/settings/archive", async (c) => {
  ...
  if (!authParsed.success) {
    return c.json(
      { error: "Invalid auth envelope", details: authParsed.error.flatten() },
      400,
    );
  }
  try {
    const settings = archiveProject(
      projectId,
      authParsed.data.auth,
      "current-user",
    );
    return c.json({ settings });
  } catch (err) {
...
projectSettingsRoutes.post("/:projectId/settings/transfer", async (c) => {
  ...
  try {
    const settings = transferProject(
      projectId,
