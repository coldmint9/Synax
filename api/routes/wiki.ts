...
import {
  getLatestWikiSnapshotTree,
  publishLatestWikiSnapshot,
  WikiSnapshotEventReason,
} from "../services/wiki/wiki-snapshot-events.js";
...
function projectWorkDir(projectId: string, requested?: string): string {
  return resolveProjectWorkspaceLocation(projectId)
    ? resolveProjectWorkDir(projectId)
    : (requested ?? "");
}
...
wikiRoutes.patch("/documents/:documentId", async (c) => {
  ...
  try {
    ...
    await publishLatestWikiSnapshot(
      updated.projectId,
      WikiSnapshotEventReason.DocumentUpdated,
    );
...
wikiRoutes.get("/snapshots/:snapshotId/export.md", async (c) => {
  ...
  try {
    const result = await wikiExportService.exportSnapshot(snapshotId, {
      includeSourceRefs,
    });
...
wikiRoutes.get("/documents/:documentId/export.md", async (c) => {
  ...
  const includeSourceRefs = c.req.query("refs") === "1";
  try {
    const result = await wikiExportService.exportDocument(documentId, {
      includeSourceRefs,
    });
    c.header("Content-Type", "text/markdown; charset=utf-8");
    c.header(
...
wikiRoutes.post("/projects/:projectId/generate", async (c) => {
  ...
  try {
    ...
    if (err instanceof AgentProviderNotConfiguredError) {
      logger.warn(
        { projectId },
        "[wiki] LLM provider not configured for generate",
      );
  ...
  setImmediate(() => {
    ...
    if (!started) {
      logger.warn(
        { projectId },
        "[wiki] failed to start wiki job child process",
      );
    }
  });
...
wikiRoutes.post("/projects/:projectId/reinitialize", async (c) => {
  ...
  try {
    ...
    if (err instanceof AgentProviderNotConfiguredError) {
      logger.warn(
        { projectId },
        "[wiki] LLM provider not configured for reinitialize",
      );
      return c.json({ error: err.message, code: err.code }, 422);
    }
    return c.json(
  ...
  return c.json({
    status: "queued",
    message: "Wiki purge and regeneration queued.",
  });
});
...
wikiRoutes.post("/snapshots/:snapshotId/continue", async (c) => {
  ...
  return c.json({
    status: "queued",
    message: "Wiki continue generation started.",
  });
});
...
wikiRoutes.post("/snapshots/:snapshotId/approve", async (c) => {
  ...
  if (snapshot.status !== "outline_ready") {
    return c.json(
      {
        error: `Snapshot must be outline_ready to approve, got "${snapshot.status}"`,
      },
      409,
    );
  }
  ...
  return c.json({
    status: "queued",
    message: "Wiki content generation started.",
  });
});
...
wikiRoutes.post("/snapshots/:snapshotId/pause", async (c) => {
  ...
  if (snapshot.status !== "writing") {
    return c.json(
      { error: `Snapshot must be writing to pause, got "${snapshot.status}"` },
      409,
    );
  }
  ...
  try {
    ...
    return c.json({
      status: "paused",
      message: "Wiki content generation paused.",
    });
  } catch (err) {
    return c.json(
...
wikiRoutes.post("/snapshots/:snapshotId/refresh", async (c) => {
  ...
  try {
    ...
    if (err instanceof AgentProviderNotConfiguredError) {
      logger.warn(
        { snapshotId, projectId: snapshot.projectId },
        "[wiki] LLM provider not configured for refresh",
      );
      return c.json({ error: err.message, code: err.code }, 422);
    }
    return c.json(
  ...
  try {
    const task = await wikiRefreshService.triggerRefresh(
      snapshot.projectId,
      snapshotId,
      projectWorkDir(snapshot.projectId, parsed.data.workDir),
      parsed.data.locale,
    );
    return c.json({ task });
  } catch (err) {
...
wikiRoutes.post("/drafts/:draftId/apply", async (c) => {
  ...
  try {
    ...
    if (draft)
      await publishLatestWikiSnapshot(
        draft.projectId,
        WikiSnapshotEventReason.DraftApplied,
      );
...
wikiRoutes.post("/drafts/:draftId/apply-partial", async (c) => {
  ...
  if (!parsed.ok) return c.json({ error: parsed.error }, 400);
  try {
    const result = await wikiDraftService.applyPartial(
      draftId,
    ...
    return c.json(
      { error: err instanceof Error ? err.message : "apply-partial failed" },
      400,
    );
  }
});
...
wikiRoutes.post("/drafts/:draftId/edit", async (c) => {
  ...
  if (!parsed.ok) return c.json({ error: parsed.error }, 400);
  try {
    const result = await wikiDraftService.editAndApply(
      draftId,
      parsed.data.changes,
      {
        actorId: parsed.data.actorId,
...
wikiRoutes.post("/drafts/:draftId/discard", async (c) => {
  ...
  if (!parsed.ok) return c.json({ error: parsed.error }, 400);
  try {
    const draft = await wikiDraftService.discardDraft(draftId, {
      actorId: parsed.data.actorId,
    });
    await publishLatestWikiSnapshot(
      draft.projectId,
      WikiSnapshotEventReason.DraftDiscarded,
    );
    return c.json(draft);
  } catch (err) {
    return c.json(
...
wikiRoutes.patch("/goals/:goalId/status", async (c) => {
  ...
  const parsed = await parseBody(
    c,
    z.object({
      status: z.enum(["active", "planned", "in_progress", "resolved"]),
    }),
  );
  if (!parsed.ok) return c.json({ error: parsed.error }, 400);
...
wikiRoutes.post("/projects/:projectId/plans/generate", async (c) => {
  ...
  if (existing) {
    return c.json(
      {
        error: "An active plan already exists. Complete or discard it first.",
...
wikiRoutes.post("/projects/:projectId/plans/generate/stream", async (c) => {
  ...
  if (existing) {
    return c.json(
      { error: "An active plan already exists.", code: "PLAN_ALREADY_EXISTS" },
      409,
    );
  }
  ...
  return streamSSE(c, async (stream) => {
    ...
    try {
      for await (const event of generatePlanStream(
        projectId,
        snapshotId,
        locale,
      )) {
      ...
      await stream
        .writeSSE({ data: JSON.stringify({ type: "failed", error: msg }) })
        .catch(() => {});
...
wikiRoutes.delete("/plans/:planId", async (c) => {
  ...
  if (permanent) {
    ...
    if (plan.status === "completed" || plan.status === "discarded")
      return c.json({ error: "Plan already finalized" }, 409);
...
wikiRoutes.post("/plans/:planId/nodes", async (c) => {
  ...
  if (plan.status !== "draft")
    return c.json({ error: "Plan is not in draft" }, 409);
  const parsed = await parseBody(
    c,
    z.object({
...
wikiRoutes.post("/plans/:planId/nodes/:nodeId/accept", async (c) => {
  ...
  const plan = await goalService.getPlan(planId);
  if (!plan) return c.json({ error: "Plan not found" }, 404);
  await planExecutor.acceptPlanNode(
    planId,
    nodeId,
    projectWorkDir(plan.projectId, parsed.data.workDir),
  );
  return c.json({ ok: true });
});
...
wikiRoutes.post("/plans/:planId/nodes/:nodeId/redo", async (c) => {
  ...
  if (!parsed.ok) return c.json({ error: parsed.error }, 400);
  await planExecutor.redoPlanNode(
    planId,
    nodeId,
    parsed.data.feedback,
    parsed.data.locale ?? "zh",
  );
  return c.json({ ok: true });
});
