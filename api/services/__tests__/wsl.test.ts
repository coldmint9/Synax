...
describe("WSL gateway", () => {
  ...
  it("forwards only explicit non-Windows overrides through WSLENV", () => {
    ...
    expect(env.WSLENV?.split(":")).toEqual(
      expect.arrayContaining(["HTTPS_PROXY", "GIT_TERMINAL_PROMPT"]),
    );
  ...
  it("builds a parameterized direct command without shell interpolation", () => {
    expect(
      wslCommandSpec("Ubuntu Dev", "/home/me/a b", "git", [
        "status",
        "--short",
      ]),
    ).toEqual({
  ...
  it("adds an owner marker and rejects injected distribution names", () => {
    expect(
      wslCommandSpec("Ubuntu", "/repo", "npm", ["test"], {
        ownerId: "owner-1",
      }),
    ).toEqual({
      command: "wsl.exe",
      args: [
        ...
        "--exec",
        "/usr/bin/env",
        "SYNAX_PROCESS_OWNER=owner-1",
        "/usr/bin/setsid",
        "npm",
        "test",
      ],
    });
    expect(() => wslCommandSpec("Ubuntu\n--exec", "/repo", "git", [])).toThrow(
      /distribution name is invalid/i,
    );
  });

  it("uses /bin/sh only for shell commands", () => {
    expect(
      wslCommandSpec("Ubuntu", "/repo", 'printf "$HOME"', [], { shell: true }),
    ).toEqual({
      command: "wsl.exe",
      args: [
        "--distribution",
