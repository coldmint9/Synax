...
import {
  AcpPermissionBridge,
  acpPermissionBridge,
} from "../acp-engine/acp-permission-bridge.js";
...
describe("ACP permission identity and scope", () => {
  ...
  it("binds different Synax/native IDs and rejects a stale native session", async () => {
    ...
    bridge.setTurnContext(session.id, {
      sessionId: session.id,
      acpSessionId: "native-current",
      runId: makeRuntimeId("run"),
      stepId: makeRuntimeId("step"),
      rules: session.permissionRules,
      isSubSession: false,
      onPermissionRequested: requested,
    });
    const request = {
      ...
      options: [
        { optionId: "approve", kind: "allow_once" as const, name: "Approve" },
      ],
    };
    expect(
      (
        await bridge.handleRequest(
          { ...request, sessionId: "native-stale" },
          session.id,
        )
      ).outcome,
  ...
  it("never escalates an allow-once decision to persistent permission", async () => {
    ...
    const response = await bridge.handleRequest({
      ...
      options: [
        { optionId: "all", kind: "allow_always", name: "Always allow" },
      ],
    });
    expect(response.outcome).toEqual({ outcome: "cancelled" });
  });
...
describe("per-operation approval scope", () => {
  it("never upgrades a single-use approval to a provider persistent grant", async () => {
    const session = agentSessionRuntime.create({
      ...plannerSessionInput,
      permissionTier: "boundary",
    });
    ...
    const pending = bridge.handleRequest({
      sessionId: session.id,
      toolCall: {
        toolCallId: "native-persistent",
        kind: "edit",
        title: "Edit a file",
      },
      options: [{ optionId: "forever", kind: "allow_always", name: "Always" }],
    });
    await Promise.resolve();
    const decision = requested.mock.calls[0][0];
    expect(decision.metadata.allowedReplies).toEqual(["reject"]);
    expect(bridge.resolve(session.id, decision.id, "once")).toBe(false);
    expect(bridge.resolve(session.id, decision.id, "always")).toBe(false);
    expect(() =>
      permissionPolicy.reply(session.id, decision.id, "once"),
    ).toThrow();
    permissionPolicy.reply(session.id, decision.id, "reject");
    expect(bridge.resolve(session.id, decision.id, "reject")).toBe(true);
    expect((await pending).outcome).toEqual({ outcome: "cancelled" });
  });
});
