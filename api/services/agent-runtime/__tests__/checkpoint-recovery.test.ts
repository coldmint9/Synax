...
beforeEach(async () => {
  resetAgentRuntimeFixtures();
  root = await fs.mkdtemp(path.join(os.tmpdir(), "synax-recovery-"));
  id = create(root);
});
afterEach(async () => {
  vi.restoreAllMocks();
...
describe("history recovery safety", () => {
  it("compensates applied files when a later file write fails without truncating history", async () => {
    await fs.writeFile(path.join(root, "a"), "before-a");
    await fs.writeFile(path.join(root, "b"), "before-b");
    const cp = await checkpoint();
    await withCheckpointMutation(id, async () => {
      await fs.writeFile(path.join(root, "a"), "after-a");
      await fs.writeFile(path.join(root, "b"), "after-b");
    });
    const realWrite = checkpointFiles.write.bind(checkpointFiles);
    let calls = 0;
    ...
    expect(await fs.readFile(path.join(root, "a"), "utf8")).toBe("after-a");
    expect(await fs.readFile(path.join(root, "b"), "utf8")).toBe("after-b");
    expect(historyRevision(id)).toBe(0);
  ...
  it("recovers a journal left between a filesystem write and DB commit", async () => {
    ...
    const before = await checkpointFiles.version(canonical, "a", true);
    await fs.writeFile(path.join(root, "a"), "new");
    const after = await checkpointFiles.version(canonical, "a", true),
      changes = [{ root: canonical, path: "a", before, after }];
    const db = getRawSqlite();
    db.prepare(
      "INSERT INTO conversation_workspace_locks(root,operation_id) VALUES (?,'crashed')",
    ).run(canonical);
    db.prepare(
      "INSERT INTO conversation_history_operations(id,session_id,request_hash,state,payload_json,created_at) VALUES ('crashed',?,'hash','applying',?,?)",
    ).run(
      id,
      JSON.stringify({
  ...
  it("keeps different paths parallel but fences concurrent writes to the same path", async () => {
    let release!: () => void, entered!: () => void;
    const gate = new Promise<void>((r) => (release = r)),
      started = new Promise<void>((r) => (entered = r));
    const first = recordFileChange(
      id,
      async () => {
        entered();
        await gate;
    ...
    release();
    await first;
    expect(await fs.readFile(path.join(root, "a"), "utf8")).toBe("one");
  });
  it("does not undo a different session or manual editor, even if the resulting bytes match", async () => {
    ...
    await withCheckpointMutation(other, () =>
      fs.writeFile(path.join(root, "file"), "agent"),
    );
    expect(
      (await previewHistory(id, cp.id)).conflicts.some((c) =>
        c.reason.includes("Another session"),
      ),
    ).toBe(true);
  });
  it("keeps published checkpoints immutable after later writes", async () => {
    ...
    const cp = await checkpoint();
    await withCheckpointMutation(id, () =>
      fs.writeFile(path.join(root, "file"), "later"),
    );
    const repeated = await captureCheckpoint(id, "reply", "reply");
    expect(repeated).toEqual(cp);
  ...
  it("forks dirty Git files without changing source HEAD, index or working files", async () => {
    ...
    git("init", "-b", "main");
    await fs.writeFile(path.join(root, "tracked"), "committed");
    git("add", "tracked");
    git(
      "-c",
      "user.name=Test",
    ...
    await fs.writeFile(path.join(root, "tracked"), "historical dirty");
    await fs.writeFile(path.join(root, "new"), "historical new");
    const cp = await checkpoint();
    await fs.writeFile(path.join(root, "tracked"), "present");
    git("add", "tracked");
    const sourceHead = git("rev-parse", "HEAD"),
      index = git("write-tree");
    const result = await forkCheckpoint(id, cp.id, 0, "git-fork");
    const fork = store.getSession(result.sessionId),
      directory = (fork.sessionMetadata?.backend as { workDir: string })
        .workDir;
    try {
      expect(await fs.readFile(path.join(directory, "tracked"), "utf8")).toBe(
        "present",
      );
      ...
      expect(await fs.readFile(path.join(root, "tracked"), "utf8")).toBe(
        "present",
      );
      expect(
