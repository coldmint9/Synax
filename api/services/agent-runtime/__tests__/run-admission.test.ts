...
describe("durable Run admission", () => {
  ...
  it("returns the same persisted Run for a repeated request without executing it", () => {
    ...
    const first = acceptRuntimeRun(
      session.id,
      { message: "Read README" },
      "request-a",
    );
    const replay = acceptRuntimeRun(
      session.id,
      { message: "Read README" },
      "request-a",
    );
  ...
  it("rejects reusing an idempotency key for different input", () => {
    ...
    expect(() =>
      acceptRuntimeRun(session.id, { message: "Delete README" }, "request-a"),
  ...
  it("allows only one accepted execution per session until the current Run settles", () => {
    ...
    expect(() =>
      acceptRuntimeRun(session.id, { message: "two" }, "two"),
    ).toThrow(/active|queued/i);
  ...
  it("activates the reserved Run exactly once instead of creating a second one", () => {
    ...
    const started = activateAcceptedRun(
      session.id,
      run.id,
      "user-message",
      null,
    ...
    expect(agentRuntimeStore.listRuns(session.id)).toHaveLength(1);
    expect(() =>
      activateAcceptedRun(session.id, run.id, "again", null),
    ).toThrow(/queued|active/i);
  });
  ...
  it("rejects an activation under the wrong session identity", () => {
    ...
    expect(() => activateAcceptedRun(other.id, run.id, "wrong", null)).toThrow(
      /session/i,
    );
  ...
  it("marks a queued execution interrupted on restart instead of silently replaying it", () => {
    ...
    expect(
      acceptRuntimeRun(session.id, { message: "Read" }, "restart"),
    ).toMatchObject({
      reused: true,
      run: { id: accepted.run.id, status: "interrupted" },
    });
  });
  ...
  it("rejects non-native execution backends for WSL2 workspaces", () => {
    ...
    expect(() =>
      acceptRuntimeRun(session.id, { message: "Read" }, "wsl-external"),
