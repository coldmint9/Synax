...
describe('headless Run coordinator', () => {
  it('waits for completion, then starts queued messages as separate FIFO runs without an observer', async () => {
    ...
    const first = coordinator.submit(session.id, { message: 'Original' }, 'queue-original');
    inputQueueService.enqueue(session.id, { message: 'Second', reasoningEffort: 'high' });
    ...
    expect(execute.mock.calls.map(call => call[2].message)).toEqual(['Original', 'Second', 'Third']);
    expect(execute.mock.calls[1][2].reasoningEffort).toBe('high');
    expect(new Set(execute.mock.calls.map(call => call[2].acceptedRunId)).size).toBe(3);
  ...
  it.each([
    { label: 'round handoff', runStatus: 'completed', sessionStatus: 'completed', stopReason: 'round_yielded' },
    { label: 'failed', runStatus: 'failed', sessionStatus: 'failed', stopReason: 'failed' },
    { label: 'blocked run', runStatus: 'blocked', sessionStatus: 'completed', stopReason: 'blocked' },
    { label: 'waiting_input', runStatus: 'waiting_input', sessionStatus: 'waiting_input', stopReason: 'waiting_input' },
    { label: 'waiting_permission', runStatus: 'waiting_permission', sessionStatus: 'waiting_permission', stopReason: 'waiting_permission' },
    { label: 'cancelled', runStatus: 'cancelled', sessionStatus: 'cancelled', stopReason: 'cancelled' },
    { label: 'interrupted', runStatus: 'interrupted', sessionStatus: 'interrupted', stopReason: 'interrupted' },
  ] as const)(
    'keeps queued messages when the conversation ends in $label', async ({ runStatus, sessionStatus, stopReason }) => {
      const session = create();
      execute.mockImplementationOnce(async function* (id, _mode, input) {
        ...
        agentRuntimeStore.updateSession(id, { status: sessionStatus, activeRunId: null });
        yield { type: 'done', sessionId: id, runId: finished.id };
      });
      coordinator.submit(session.id, { message: 'Original' }, `queue-${runStatus}`);
  ...
  it('dispatches an input arriving after completion and does not submit it twice', async () => {
    ...
    expect(coordinator.dispatchQueuedInput(session.id)).toBe(false);
    await coordinator.waitForIdle();
    expect(execute.mock.calls.map(call => call[2].message)).toEqual(['Original', 'Late input']);
  });
  ...
  it('rolls back admission if removing the queue item fails', async () => {
    ...
    const take = vi.spyOn(inputQueueService, 'take').mockImplementationOnce(() => { throw new Error('Queue write failed'); });
    try {
      expect(coordinator.dispatchQueuedInput(session.id)).toBe(false);
      expect(inputQueueService.list(session.id)).toHaveLength(1);
