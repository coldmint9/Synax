...
function insertRawSession(input: {
  ...
  updatedAt?: string;
  status?: AgentSession["status"] | "blocked" | "paused";
  profileId?: string;
...
describe("listSessions SQL scoping", () => {
  ...
  it("applies project/node/status filters with the previous ordering and composition", () => {
    ...
    expect(agentRuntimeStore.listSessions().map((s) => s.id)).toEqual([
      "s1",
      "s2",
      "s3",
    ]);
    expect(
      agentRuntimeStore.listSessions({ projectId: "p1" }).map((s) => s.id),
    ).toEqual(["s1", "s2"]);
    expect(
      agentRuntimeStore.listSessions({ nodeId: "n1" }).map((s) => s.id),
    ).toEqual(["s1", "s3"]);
    expect(
      agentRuntimeStore.listSessions({ status: "running" }).map((s) => s.id),
    ).toEqual(["s1", "s3"]);
    expect(
      agentRuntimeStore
        .listSessions({ projectId: "p1", status: "running" })
        .map((s) => s.id),
    ).toEqual(["s1"]);
    expect(
      agentRuntimeStore
        .listSessions({ projectId: "p1", nodeId: "n2", status: "completed" })
        .map((s) => s.id),
    ).toEqual(["s2"]);
    expect(
      agentRuntimeStore.listSessions({ nodeId: "n1", status: "completed" }),
    ).toEqual([]);
  });
  ...
  it("slices to the most recently updated rows without loading the rest", () => {
    for (let i = 0; i < 60; i += 1) {
      ...
      agentRuntimeStore.createSession(
        sessionFixture({
          id: `s${stamp}`,
          updatedAt: `2026-01-01T00:00:${stamp}.000Z`,
        }),
      );
    }

    const defaultPage = agentRuntimeStore.listSessions({
      projectId: "project-alpha",
    });
    ...
    expect(
      agentRuntimeStore
        .listSessions({ projectId: "project-alpha", limit: 3 })
        .map((s) => s.id),
    ).toEqual(["s59", "s58", "s57"]);
    expect(
      agentRuntimeStore.listSessions({ projectId: "project-alpha", limit: 0 }),
    ).toEqual([]);
    expect(
  ...
  it("keeps Array#slice semantics for negative limits", () => {
    for (let i = 1; i <= 4; i += 1) {
      agentRuntimeStore.createSession(
        sessionFixture({
          id: `s${i}`,
          updatedAt: `2026-01-01T00:00:0${i}.000Z`,
        }),
      );
    }
    const all = agentRuntimeStore.listSessions({
      limit: Number.MAX_SAFE_INTEGER,
    });
    expect(
      agentRuntimeStore.listSessions({ limit: -1 }).map((s) => s.id),
    ).toEqual(all.slice(0, -1).map((s) => s.id));
    expect(
      agentRuntimeStore.listSessions({ limit: -2 }).map((s) => s.id),
    ).toEqual(all.slice(0, -2).map((s) => s.id));
  });

  it("treats empty-string filters as absent", () => {
    agentRuntimeStore.createSession(
      sessionFixture({ id: "s1", projectId: "p1", nodeId: "n1" }),
    );
    expect(
      agentRuntimeStore
        .listSessions({ projectId: "", nodeId: "", status: "" })
        .map((s) => s.id),
    ).toEqual(["s1"]);
  });
  ...
  it("pushes filters and the limit into one scoped SQL statement", () => {
    agentRuntimeStore.createSession(
      sessionFixture({ id: "s1", projectId: "p1", status: "running" }),
    );
    agentRuntimeStore.createSession(
      sessionFixture({ id: "s2", projectId: "p2", status: "running" }),
    );
    ...
    const sessionQueries = statements.filter((entry) =>
      entry.sql.includes("FROM agent_runtime_sessions"),
    );
...
describe("listSessionTree traversal", () => {
  ...
  it("returns a pre-order tree and never touches unrelated sessions", () => {
    agentRuntimeStore.createSession(
      sessionFixture({ id: "root", updatedAt: "2026-01-01T00:00:10Z" }),
    );
    agentRuntimeStore.createSession(
      sessionFixture({
        id: "c1",
    ...
    getRawSqlite()
      .prepare(
        "UPDATE agent_runtime_sessions SET child_session_ids_json = ? WHERE id = ?",
      )
      .run(JSON.stringify(["c1", "c2", "legacy-child"]), "root");
    // Unrelated sessions must be excluded from the walk.
    agentRuntimeStore.createSession(
      sessionFixture({ id: "unrelated", updatedAt: "2026-01-01T00:00:20Z" }),
    );
    insertRawSession({
      id: "unrelated-child",
  ...
  it("uses a single recursive CTE instead of loading every session", () => {
    ...
    agentRuntimeStore.createSession(
      sessionFixture({ id: "child", parentSessionId: "root" }),
    );

    const { result, statements } = captureSql(() =>
      agentRuntimeStore.listSessionTree("root"),
    );
    ...
    expect(query!.sql).not.toContain(
      "SELECT * FROM agent_runtime_sessions ORDER BY updated_at DESC",
    );
  ...
  it("terminates on cycles in parent/child links", () => {
    insertRawSession({
      id: "a",
      parentSessionId: null,
      childSessionIdsJson: JSON.stringify(["b"]),
    });
    insertRawSession({
      id: "b",
      parentSessionId: "a",
      childSessionIdsJson: JSON.stringify(["a"]),
    });

    expect(agentRuntimeStore.listSessionTree("a").map((s) => s.id)).toEqual([
      "a",
      "b",
    ]);
  });
  ...
  it("treats malformed and non-array child lists as empty", () => {
    ...
    expect(
      agentRuntimeStore.listSessionTree("malformed").map((s) => s.id),
    ).toEqual(["malformed"]);
    expect(
      agentRuntimeStore.listSessionTree("object-list").map((s) => s.id),
    ).toEqual(["object-list", "valid-child"]);
  });

  it("throws when the root session does not exist", () => {
    expect(() => agentRuntimeStore.listSessionTree("missing-root")).toThrow(
      AgentNotFoundError,
    );
  });
});
...
describe("listSessionsPage projected-status pagination", () => {
  ...
  it("reports exact totalCount and countByStatus over the projected status", () => {
    ...
    expect(page.countByStatus).toEqual({
      completed: 3,
      stopping: 1,
      running: 1,
    });
  ...
  it("filters by projected status, matching the stopping/unconfirmed rewrite", () => {
    ...
    expect(
      agentRuntimeStore.listSessionsPage(
        { status: "stopping" },
        { limit: 50, offset: 0 },
      ),
    ).toMatchObject({
    ...
    expect(
      agentRuntimeStore
        .listSessionsPage({ status: "stopping" }, { limit: 50, offset: 0 })
        .items.map((s) => s.id),
    ).toEqual(["stopping"]);
    expect(
      agentRuntimeStore
        ...
        .items.map((s) => s.id),
    ).toEqual(["c-new", "unconfirmed", "c-old"]);
    // The persisted `running` status must not leak the two rewritten rows.
    expect(
      agentRuntimeStore
        .listSessionsPage({ status: "running" }, { limit: 50, offset: 0 })
        .items.map((s) => s.id),
    ).toEqual(["running"]);
  });

  it("normalizes legacy Synax terminal rows and user-waiting states into public statuses", () => {
    insertRawSession({
      id: "synax-paused",
      profileId: "synax",
      status: "paused",
    ...
    expect(
      agentRuntimeStore.listSessionsPage({}, { limit: 50, offset: 0 })
        .countByStatus,
    ).toEqual({ completed: 2, waiting_permission: 1 });
    expect(
      agentRuntimeStore.listSessionsPage(
        { status: "blocked" },
        { limit: 50, offset: 0 },
  ...
  it("pages with the updated_at DESC order and clamps a non-positive window", () => {
    ...
    expect(
      agentRuntimeStore
        .listSessionsPage({}, { limit: 2, offset: 2 })
        .items.map((s) => s.id),
    ).toEqual(["unconfirmed", "running"]);
    expect(
      agentRuntimeStore
        .listSessionsPage({}, { limit: 2, offset: 4 })
        .items.map((s) => s.id),
    ).toEqual(["c-old"]);
    expect(
      agentRuntimeStore.listSessionsPage({}, { limit: 2, offset: 99 }).items,
    ).toEqual([]);
    // SQLite treats LIMIT -1 as unbounded; the guard must keep it empty.
    expect(
      agentRuntimeStore.listSessionsPage({}, { limit: -1, offset: 0 }).items,
    ).toEqual([]);
    expect(
      agentRuntimeStore.listSessionsPage({}, { limit: 0, offset: 0 }).items,
    ).toEqual([]);
  });
  ...
  it("falls back to the persisted status when session_metadata_json is malformed", () => {
    insertRawSession({
      id: "bad-meta",
      status: "running",
      sessionMetadataJson: "not json",
      updatedAt: "2026-01-01T00:00:01Z",
    });
    insertRawSession({
      id: "null-safe",
      status: "completed",
      sessionMetadataJson: null as unknown as string,
      updatedAt: "2026-01-01T00:00:02Z",
    });
  ...
  it("resolves status filtering and paging without materializing every session", () => {
    ...
    const { result, statements } = captureSql(() =>
      agentRuntimeStore.listSessionsPage(
        { projectId: "project-alpha", status: "running" },
        { limit: 1, offset: 0 },
      ),
    );
...
describe("getSessionStats active sub-agent counting", () => {
  ...
  it("counts running children with one status lookup and ignores missing ids", () => {
    const parent = agentRuntimeStore.createSession(
      sessionFixture({ id: "parent", status: "running" }),
    );
    agentRuntimeStore.createSession(
      sessionFixture({
        id: "run-child",
    ...
    const { result, statements } = captureSql(() =>
      agentRuntimeStore.getSessionStats(parent.id),
    );

    expect(result.activeSubAgentCount).toBe(2);
    const statusQueries = statements.filter((entry) =>
      entry.sql.includes("status = 'running' AND id IN"),
    );
    expect(statusQueries).toHaveLength(1);
    expect(statusQueries[0]!.args).toEqual([
      "run-child",
      "done-child",
      "deleted-child",
    ]);
