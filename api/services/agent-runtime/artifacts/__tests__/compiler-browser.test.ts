...
const executablePath = [process.env.SYNAX_ARTIFACT_BROWSER, '/Applications/Google Chrome.app/Contents/MacOS/Google Chrome', '/usr/bin/chromium', '/usr/bin/google-chrome'].find(p => p && fs.existsSync(p));
...
describe.runIf(Boolean(executablePath))('real offline artifact browser', () => {
  ...
  beforeAll(async () => { root = fs.mkdtempSync(path.join(os.tmpdir(),'artifact-browser-')); browser = await chromium.launch({executablePath,headless:true}); });
  afterAll(async () => { await browser?.close(); fs.rmSync(root,{recursive:true,force:true}); });
  it('runs real SDK before HTML scripts, preserves local controls/state, and blocks connect-src', async () => {
    fs.writeFileSync(path.join(root,'index.html'), `<button id="counter" style="color:red">0</button><script>const sharedValue=42;function sharedFunction(){return sharedValue}</script><script>
      ...
      synaxWidget.ready().then(async x=>{window.standalone=x.standalone;await synaxWidget.setState({privateState:{n:7}});window.saved=synaxWidget.getState().privateState.n;});
      fetch('https://artifact-egress.invalid/secret').then(()=>window.blocked=false,()=>window.blocked=true);
    </script>`);
    const result = await compileArtifact(readSnapshot(root,'index.html'),'html');
    ...
    let network = 0; await page.route('https://artifact-egress.invalid/**',route=>{network++;return route.abort();});
    await page.goto(pathToFileURL(path.join(root,'preview.html')).href);
    await page.waitForFunction('window.saved===7 && window.blocked===true');
    expect(await page.evaluate('window.bootstrapPresent && window.standalone')).toBe(true);
    expect(await page.evaluate('window.sharedScriptValue')).toBe(42);
    await page.getByRole('button',{name:'0'}).click(); expect(await page.getByRole('button').textContent()).toBe('1');
    expect(await page.getByRole('button').evaluate(el=>getComputedStyle(el).color)).toBe('rgb(255, 0, 0)');
    expect(network).toBe(0); expect(errors).toEqual([]); await page.close();
  });
  it('runs TSX with real React and retains interaction under hash-only CSP', async () => {
    fs.writeFileSync(path.join(root,'app.tsx'),`import React from 'react';export default function App(){const [n,setN]=React.useState(0);return <button style={{color:'blue'}} onClick={()=>setN(n+1)}>Count {n}</button>}`);
    const result = await compileArtifact(readSnapshot(root,'app.tsx'),'react');
    ...
    await page.getByRole('button',{name:'Count 0'}).click(); await page.getByRole('button',{name:'Count 1'}).waitFor();
    expect(await page.getByRole('button').evaluate(el=>getComputedStyle(el).color)).toBe('rgb(0, 0, 255)');
  ...
  it('renders and updates a D3 bar chart entirely offline under the unchanged CSP', async () => {
    fs.writeFileSync(path.join(root,'chart.html'), `<svg aria-label="D3 chart" width="240" height="100"></svg><button>Update chart</button><script type="module">
    ...
    const result=await compileArtifact(readSnapshot(root,'chart.html'),'html');
    expect(result.dependencies.find(dep=>dep.name==='d3')?.version).toBe('7.9.0');
    expect(result.html).not.toContain("'unsafe-eval'");
    fs.writeFileSync(path.join(root,'chart-preview.html'),result.html);
    const page=await browser.newPage(); const errors:string[]=[]; let remoteRequests=0;
    page.on('pageerror',error=>errors.push(error.message));
    await page.route(/^https?:/,route=>{remoteRequests++; return route.abort();});
    await page.goto(pathToFileURL(path.join(root,'chart-preview.html')).href);
    const bars=page.locator('svg rect'); await bars.first().waitFor();
    expect(await bars.evaluateAll(elements=>elements.map(el=>el.getAttribute('width')))).toEqual(['40','100','200']);
    await page.getByRole('button',{name:'Update chart'}).click();
    expect(await bars.evaluateAll(elements=>elements.map(el=>el.getAttribute('width')))).toEqual(['60','120','180']);
