...
import { ensureSessionTitleGenerated, maybeScheduleSessionTitleFromStreamChunk } from './session-title-service.js';

/** The caller owns this signal. A browser observer must never pass its connection signal here. */
export async function* executeBackendSession(sessionId: string, mode: AgentSessionStreamMode, input: StreamTurnRequest, signal?: AbortSignal): AsyncGenerator<AgentRunStreamChunk> {
  ...
  } finally { ensureSessionTitleGenerated(sessionId); }
}
