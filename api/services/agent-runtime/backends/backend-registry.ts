...
import { BACKENDS, type BackendAdapter, type BackendId } from './backend-contracts.js';
...
const native: BackendAdapter = {
  async *stream(sessionId, mode, input, signal) {
    if (process.env.SYNAX_AGENT_SESSION_IN_PROCESS !== '1') {
      ...
      yield* agentLoopRuntime.streamRun(sessionId, input, signal, mode === 'resume');
    }
  },
  async interrupt(sessionId, reason) {
    await sessionProcessManager.interruptAndWaitForSessions([sessionId], reason);
    await agentLoopRuntime.interruptAndWaitForSessions([sessionId], reason);
  },
  async close(sessionId) { await native.interrupt(sessionId, 'Session closed.'); },
};
const acp: BackendAdapter = {
  async *stream(sessionId, mode, input, signal) {
    for await (const chunk of acpSessionEngine.stream(sessionId, mode, input, signal)) {
      forwardChunkToLiveBus(sessionId, chunk);
      yield chunk;
    }
  },
  interrupt: (sessionId, reason) => acpSessionEngine.interruptSession(sessionId, reason),
  close: (sessionId) => acpSessionEngine.closeSession(sessionId),
  hasPendingPermission: (sessionId, id) => acpPermissionBridge.hasPendingPermission(sessionId, id),
  replyPermission(sessionId, id, reply) {
    const resolved = acpPermissionBridge.resolve(sessionId, id, reply);
    if (resolved && !acpPermissionBridge.hasPendingForSession(sessionId)) agentRuntimeStore.updateSession(sessionId, { status: 'running', pendingResumeToken: null });
    return resolved;
  },
};
...
export function describeBackends() { return BACKENDS; }
