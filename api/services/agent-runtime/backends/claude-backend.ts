...
const category = (name: string): CapabilityCategory =>
  name === "Bash"
    ? "shell"
    : /^(Read|Glob|Grep|WebSearch|WebFetch)$/.test(name)
      ? "read"
      : /^(Edit|Write|NotebookEdit)$/.test(name)
        ? "write"
        : "task";
interface Active {
  turn: ExternalTurn;
  ...
  task: Promise<void>;
  inputs: AsyncQueue<SDKUserMessage>;
  initialized: boolean;
}
export class ClaudeBackend implements BackendAdapter {
  ...
  async *stream(
    ...
    if (input.maxSteps || input.maxTokens || input.temperature !== undefined)
      throw new Error(
        "Claude controls its own agent loop; Native Synax limits and temperature overrides are unsupported.",
      );
    if (input.contentParts?.some((p) => p.type !== "text"))
    ...
    entry.task = this.run(entry, input)
      .catch((error) =>
        turn.finish(
  ...
  private async run(entry: Active, input: StreamTurnRequest): Promise<void> {
    ...
    const { options, context } = claudeOptions(turn.workDir, (process) => {
    ...
    options.sandbox = {
      ...
      filesystem: {
        ...
        allowWrite: turn.workspace.writableRoots,
      },
    };
    options.systemPrompt = {
      type: "preset",
      preset: "claude_code",
      append: turn.workspace.prompt,
    };
    const { query } = await import("@anthropic-ai/claude-agent-sdk");
    if (entry.controller.signal.aborted)
      throw new Error("Claude execution interrupted.");
    const model =
      input.model && input.model !== "default" ? input.model : context.model;
    const reportedUsage = new Map<
      string,
      {
        inputTokens: number;
        outputTokens: number;
    ...
    entry.query = query({
      ...
      options: {
        ...options,
        model,
        ...(native.id === "claude-code" && typeof native.sessionId === "string"
          ? { resume: native.sessionId }
          : {}),
        ...(input.reasoningEffort && input.reasoningEffort !== "none"
          ? { effort: input.reasoningEffort }
          : {}),
        canUseTool: (name, args, request) =>
          this.permission(entry, name, args, request),
      },
    });
    try {
      for await (const message of entry.query) {
        ...
        if (message.type === "system" && message.subtype === "init") {
          if (
            ...
            throw new Error(
              `Claude did not confirm isolated configuration: ${JSON.stringify({ permissionMode: message.permissionMode, mcp: message.mcp_servers.map((server) => server.name), plugins: message.plugins.map((plugin) => plugin.name), skills: message.skills })}`,
            );
          if (
            native.id === "claude-code" &&
            native.sessionId &&
            native.sessionId !== message.session_id
          )
            throw new Error(
              "Claude could not resume the original native session.",
            );
          entry.initialized = true;
          turn.model(message.model);
          store.updateSessionMetadata(turn.sessionId, {
            nativeBackend: {
              id: "claude-code",
              ...
              disabledTools: options.disallowedTools,
              tools: message.tools,
        ...
        if (message.type === "stream_event") {
          if (message.parent_tool_use_id) {
            turn.progress("Claude subagent stream", {
          ...
          if (event.type === "content_block_delta") {
            if (event.delta.type === "text_delta") {
              turn.delta(event.delta.text);
              hadText = true;
              streamed.add(`${currentMessage}:text`);
            }
            if (event.delta.type === "thinking_delta") {
              turn.delta(event.delta.thinking, true);
              streamed.add(`${currentMessage}:thinking`);
            }
          }
          ...
          if (message.message.usage && message.message.id) {
            ...
            reportedUsage.set(message.message.id, {
              inputTokens:
                usage.input_tokens +
                (usage.cache_read_input_tokens ?? 0) +
                (usage.cache_creation_input_tokens ?? 0),
            ...
            const total = [...reportedUsage.values()].reduce(
              (sum, current) => ({
                inputTokens: sum.inputTokens + current.inputTokens,
                outputTokens: sum.outputTokens + current.outputTokens,
                cachedInputTokens:
                  sum.cachedInputTokens + current.cachedInputTokens,
                cacheWriteTokens:
                  sum.cacheWriteTokens + current.cacheWriteTokens,
              }),
              {
                inputTokens: 0,
                outputTokens: 0,
          ...
          for (const block of message.message.content) {
            if (block.type === "tool_use")
              ...
              block.type === "text" &&
              !streamed.has(`${message.message.id}:text`)
            ) {
              turn.delta(block.text);
              hadText = true;
            } else if (
              !message.parent_tool_use_id &&
              block.type === "thinking" &&
              !streamed.has(`${message.message.id}:thinking`)
            )
              turn.delta(block.thinking, true);
          }
          if (message.error)
            turn.progress("Claude response error", { code: message.error });
        } else if (message.type === "user") {
          for (const block of list(message.message.content).map(object))
          ...
          const contextWindow =
            message.modelUsage?.[contextModel]?.contextWindow;
          if (contextWindow)
            turn.contextUsage({ contextWindowSize: contextWindow });
          ...
          if (message.subtype === "success" && !hadText)
            turn.delta(message.result);
          const error =
            message.is_error || message.subtype !== "success"
              ...
              : undefined;
          await turn.finish(error ? context.redact(error) : undefined);
          completed = true;
          break;
      ...
      if (!completed)
        throw new Error(
          entry.controller.signal.aborted
            ? "Claude execution interrupted."
            : "Claude exited before completing its turn.",
        );
    } catch (error) {
      throw new Error(context.redact(error));
    }
  }
  private async permission(
    ...
    if (
      ...
      return { behavior: "deny", message: "The native turn is not available." };
    const abort = () => turn.cancelPending();
    request.signal.addEventListener("abort", abort, { once: true });
    try {
      if (name === "AskUserQuestion") {
        const nativeQuestions = list(input.questions).map(object);
        const questions: HumanQuestion[] = nativeQuestions.map(
          (question, index) => ({
            id: `q${index}`,
            type:
            ...
            options: list(question.options).map((option) => ({
              value: text(object(option).label),
        ...
        const answers = Object.fromEntries(
          nativeQuestions.map((question, index) => {
            const value = answer.answers?.[`q${index}`];
            return [
      ...
      const reason = [
        request.title || request.decisionReason || `Approve Claude ${name}?`,
        request.description,
        request.blockedPath,
      ]
        .filter(Boolean)
        .join(" · ");
      const reply = await turn.permission(tool, reason, {
        tool: name,
        input,
  ...
  private progress(turn: ExternalTurn, message: SDKMessage): void {
    if (message.type === "system" && message.subtype === "api_retry")
      turn.progress("Claude API retry", {
      ...
      turn.progress("Claude tool progress", {
      ...
      turn.progress(`Claude ${message.subtype}`, {
        taskId: data.task_id,
  ...
  hasPendingPermission(sessionId: string, id: string): boolean {
    return this.active.get(sessionId)?.turn.hasPermission(id) ?? false;
  }
  replyPermission(
    sessionId: string,
    id: string,
    ...
    return this.active.get(sessionId)?.turn.replyPermission(id, reply) ?? false;
  }
  hasPendingInteraction(sessionId: string, id: string): boolean {
    return this.active.get(sessionId)?.turn.hasInput(id) ?? false;
  }
  replyInteraction(sessionId: string, id: string): boolean {
    return this.active.get(sessionId)?.turn.replyInput(id) ?? false;
  }
  async models() {
    const directory = path.join(DATA_ROOT, "native-claude-discovery");
    fs.mkdirSync(directory, { recursive: true });
    let process: ManagedProcess | undefined;
    const { options, context } = claudeOptions(directory, (child) => {
      process = child;
    });
    const { query } = await import("@anthropic-ai/claude-agent-sdk");
    const inputs = new AsyncQueue<SDKUserMessage>();
    const connection = query({ prompt: inputs, options });
    try {
      const models = await withDeadline(
        connection.supportedModels(),
        15_000,
