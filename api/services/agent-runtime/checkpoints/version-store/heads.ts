...
export interface PublishVersion { sessionId: string; versionId: string; expectedRevision: number; expectedEpoch: number }
interface IdempotentRequest { expectedRevision: number; requestId: string; requestHash: string }
export interface SwitchVersion extends IdempotentRequest { sessionId: string; targetVersionId: string }
export interface ForkVersion extends IdempotentRequest { sourceSessionId: string; targetSessionId: string; versionId: string }
...
function assertIdentity(value: string): void {
  if (typeof value !== "string" || !value.length || !value.isWellFormed() || value.includes("\0") || Buffer.byteLength(value) > 256)
    throw new VersionStoreError("VERSION_IDENTITY_INVALID", "Invalid session or request identity.");
}
function assertCounter(value: number, minimum = 0): void {
  if (!Number.isSafeInteger(value) || value < minimum || value >= Number.MAX_SAFE_INTEGER)
    throw new VersionStoreError("VERSION_REVISION_INVALID", "Invalid revision or epoch counter.");
}
function fingerprint(request: IdempotentRequest, fields: readonly string[]): string {
  ...
  return createHash("sha256").update(JSON.stringify([...fields, request.expectedRevision, request.requestHash])).digest("hex");
}
...
export class VersionHeads {
  ...
  read(sessionId: string): HeadState {
    ...
    const row = this.find.get(sessionId) as { version_id: string; revision: number; epoch: number } | undefined;
    if (!row) throw new VersionStoreError("VERSION_SESSION_MISSING", "Versioned session does not exist.");
    return { sessionId, versionId: row.version_id, revision: row.revision, epoch: row.epoch };
  }
  ...
  switch(request: SwitchVersion): HeadState {
    ...
    const bodyHash = fingerprint(request, ["switch", request.sessionId, request.targetVersionId]);
    return atomicVersionWrite(this.db, () => {
      const previous = this.result(request.sessionId, request.requestId, bodyHash);
      if (previous) return previous;
      const current = this.read(request.sessionId);
      this.checkRevision(current, request.expectedRevision);
      this.checkOwned(request.sessionId, request.targetVersionId);
      const result = this.advance(current, request.targetVersionId, current.epoch + 1);
  ...
  fork(request: ForkVersion): HeadState {
    ...
    const bodyHash = fingerprint(request, ["fork", request.sourceSessionId, request.targetSessionId, request.versionId]);
    return atomicVersionWrite(this.db, () => {
      const previous = this.result(request.sourceSessionId, request.requestId, bodyHash);
      if (previous) return previous;
      this.checkRevision(this.read(request.sourceSessionId), request.expectedRevision);
  ...
  private checkRevision(current: HeadState, expected: number): void {
    if (current.revision !== expected)
      throw new VersionStoreError("VERSION_REVISION_STALE", "Session revision changed; refresh the checkpoint preview.");
  }
  ...
  private result(sessionId: string, requestId: string, bodyHash: string): HeadState | undefined {
    const row = this.prior.get(sessionId, requestId) as {
      body_hash: string; result_session_id: string; result_version_id: string; result_revision: number; result_epoch: number;
    } | undefined;
    if (!row) return undefined;
    if (row.body_hash !== bodyHash)
      throw new VersionStoreError("VERSION_REQUEST_CONFLICT", "Request identity was already used for a different operation.");
    return { sessionId: row.result_session_id, versionId: row.result_version_id, revision: row.result_revision, epoch: row.result_epoch };
  }
