...
import { PAGE_BYTES, PAGE_ROWS, VersionStoreError, assertObjectId } from "./limits.js";
import { atomicVersionWrite } from "./transaction.js";
import {
  assertKey, childRef, nodeRef, readNode, writeBranches, writeLeaves,
  type Entry, type NodeRef, type TreeNode,
} from "./tree-nodes.js";

export interface TreeChange { key: string; value: string | null }
export interface TreeItem { key: string; value: string }
export interface TreePage { entries: TreeItem[]; next?: string }
export interface TreePageOptions { after?: string; limit?: number; maxBytes?: number }
...
function upperBound(items: readonly (readonly [string, ...unknown[]])[], key: string): number {
...
export class VersionTree {
  ...
  get(root: string | null, key: string): string | undefined {
    ...
    while (!("entries" in node)) {
      const child = childRef(node.children[Math.max(0, upperBound(node.children, key) - 1)], node.height);
      node = readNode(this.objects, child.hash, child);
    }
    const index = upperBound(node.entries, key) - 1;
    return index >= 0 && node.entries[index][0] === key ? node.entries[index][1] : undefined;
  }

  update(root: string | null, changes: readonly TreeChange[]): string | null {
    if (changes.length > PAGE_ROWS)
      throw new VersionStoreError("VERSION_TREE_BATCH", "Tree update batch exceeds its limit.");
    ...
    const sorted = [...dedup.values()].sort((a, b) => a.key < b.key ? -1 : a.key > b.key ? 1 : 0);
    if (!sorted.length) return root;
    return atomicVersionWrite(this.objects.db, () => {
      let refs: NodeRef[];
      if (root === null) {
        refs = writeLeaves(this.objects, sorted.filter(change => change.value !== null).map(change => [change.key, change.value!]));
      } else {
  ...
  private rewrite(ref: NodeRef, node: TreeNode, changes: readonly TreeChange[], root = false): NodeRef[] {
    if ("entries" in node) {
      const result: Entry[] = [];
      let cursor = 0;
      ...
      if (result.length === node.entries.length && result.every((entry, i) => entry[0] === node.entries[i][0] && entry[1] === node.entries[i][1])) return [ref];
    ...
    for (let i = 0; i < node.children.length; i++) {
      ...
      const start = cursor, next = node.children[i + 1]?.[0];
      while (cursor < changes.length && (next === undefined || changes[cursor].key < next)) cursor++;
      if (start === cursor) children.push(child);
      else children.push(...this.rewrite(child, readNode(this.objects, child.hash, child), changes.slice(start, cursor)));
    }
    if (children.length === node.children.length && children.every((child, i) => child.hash === node.children[i][1])) return [ref];
  ...
  page(root: string | null, options: TreePageOptions = {}): TreePage {
    ...
    if (!Number.isSafeInteger(limit) || limit < 1 || limit > PAGE_ROWS)
      throw new VersionStoreError("VERSION_PAGE_LIMIT", "Invalid tree page limit.");
    if (!Number.isSafeInteger(maxBytes) || maxBytes < EMPTY_PAGE_BYTES || maxBytes > PAGE_BYTES)
      throw new VersionStoreError("VERSION_PAGE_BUDGET", "Invalid tree page byte budget.");
    ...
    for (const [key, value] of this.walk(root, after)) {
      ...
      const addition = Buffer.byteLength(JSON.stringify(item)) + (entries.length ? 1 : 0);
      // Reserve the cursor even before we know whether a subsequent item exists.
      const withCursor = bytes + addition + 8 + Buffer.byteLength(JSON.stringify(key));
      if (entries.length === limit || withCursor > maxBytes) {
        if (!entries.length)
          throw new VersionStoreError("VERSION_PAGE_BUDGET", "Page byte budget cannot fit the next entry and cursor.");
        return { entries, next: entries.at(-1)!.key };
      }
      entries.push(item); bytes += addition;
    }
    return { entries };
  }

  private *walk(hash: string, after?: string, expected?: NodeRef): Generator<Entry> {
    ...
    if ("entries" in node) {
      ...
      const start = after === undefined ? 0 : Math.max(0, upperBound(node.children, after) - 1);
