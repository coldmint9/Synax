...
export class AgentContextBuilder {
  ...
  build(
    projectId: string,
    input: BuildContextRequest & { sessionId?: string } = {},
  ): AgentContextBundle {
    ...
    if (!input.nodeId)
      warnings.push(
        "No CoordForest node id supplied; bundle is project-level.",
      );
    try {
      ...
      if (suggestions.length === 0)
        warnings.push(
          "No stored Synax context suggestions matched this request.",
        );
    } catch (error) {
      warnings.push(
        `Synax context adapter unavailable: ${error instanceof Error ? error.message : String(error)}`,
      );
    }
    ...
    const bundle: AgentContextBundle = {
      ...
      blocks,
      citations: input.nodeId
        ? [{ id: makeRuntimeId("cite"), nodeId: input.nodeId }]
        : [],
      warnings,
      createdAt: nowIso(),
    };
  ...
  private describeContext(
    projectId: string,
    kind: NonNullable<BuildContextRequest["include"]>[number],
    nodeId: string | null,
  ): string {
    if (kind === "coord")
      return nodeId
        ? `CoordForest node ${nodeId} in project ${projectId}.`
        : `Project ${projectId} coordination context.`;
    if (kind === "memory") {
      try {
