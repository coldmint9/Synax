...
import { findWslOwnedProcess, wslCommandSpec, wslLauncherEnvironment } from "../wsl.js";
...
export function spawnOwnedProcess(
  ...
  const target = wsl
    ? wslCommandSpec(wsl.distribution, wsl.path, command, args, { shell: options.shell, ownerId: ticket.id })
    : { command, args };
    ...
    if (wsl) {
      void (async () => {
        for (let attempt = 0; attempt < 20; attempt += 1) {
          ...
          if (owned) {
            recordOwnedRuntime(ticket.id, { kind: "wsl", distribution: wsl.distribution, ...owned });
            return;
          }
