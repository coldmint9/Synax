...
export async function recoverRuntime(hostId: string): Promise<{ reviewed: number; resumable: string[] }> {
  const db = getRawSqlite();
  // Revoke business writes before inspecting or terminating old processes.
  db.prepare("UPDATE agent_runtime_runs SET metadata_json=json_set(metadata_json, '$.executionLease.closed', json('true')) WHERE json_extract(metadata_json, '$.executionLease.epoch') IS NOT NULL").run();
  const unresolved = await recoverOwnedProcesses(hostId);
  const resumable: string[] = [];
  let reviewed = 0;
  runtimeTransaction(() => {
    for (const session of agentRuntimeStore.listSessions({ limit: Number.MAX_SAFE_INTEGER })) {
      const runs = agentRuntimeStore.listRuns(session.id);
      const run = runs.find(item => ['running', 'queued', 'waiting_permission', 'waiting_input'].includes(item.status));
      const unknownProcesses = unresolved.filter(item => item.session_id === session.id);
      if (!run && !session.sessionMetadata?.runtimeControl && !unknownProcesses.length) continue;
      const backend = session.sessionMetadata?.backend as { id?: string } | undefined;
      const native = (backend?.id ?? (session.sessionMetadata?.acp ? 'acp' : 'native')) === 'native';
      let embedded = true;
      try { embedded = profileService.getForSession(session).executionHost === 'embedded'; } catch { /* Unknown profiles are not resumed automatically. */ }
      ...
      const requiresReview = phase !== 'queued' || unknownProcesses.length > 0;
      const reason = unknownProcesses.length ? 'Process shutdown remains unconfirmed. Stop the recorded process before recovery.'
        : requiresReview ? 'The runtime restarted during execution. Inspect workspace changes before continuing.' : 'The runtime restarted before this request was launched.';
      if (run) {
        agentRuntimeStore.updateRun(run.id, { status: 'interrupted', completedAt: nowIso(), stopReason: reason,
          metadata: { ...run.metadata, executionLease: { ...(run.metadata.executionLease as object ?? {}), closed: true }, recovery: { phase, at: nowIso() } } });
        db.prepare("UPDATE agent_runtime_run_steps SET status='interrupted', completed_at=?, finish_reason='server_restarted' WHERE run_id=? AND status='running'").run(nowIso(), run.id);
...
export async function acknowledgeRuntimeRecovery(sessionId: string): Promise<void> {
  const session = agentRuntimeStore.getSession(sessionId);
  const records = getRawSqlite().prepare("SELECT * FROM agent_runtime_processes WHERE session_id=? AND state<>'closed'").all(sessionId) as OwnedProcessRecord[];
  for (const record of records) if (!await stopRecordedProcess(record)) throw new AgentRuntimeError('A recorded process is still unconfirmed; recovery cannot release this workspace.', 'PROCESS_UNCONFIRMED', 409);
  if (!session.sessionMetadata?.runtimeControl) return;
  agentRuntimeStore.updateSessionMetadata(sessionId, { runtimeControl: null, lastRecoveryAcknowledgedAt: nowIso() });
  agentRuntimeStore.updateSession(sessionId, { status: 'interrupted', activeRunId: null, blockedReason: null, pendingResumeToken: null, updatedAt: nowIso() });
}
