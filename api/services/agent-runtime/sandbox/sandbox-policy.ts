...
export class SandboxPolicy {
  constructor(
    ...
    private readonly configFactory: (sessionId: string) => SandboxConfig = (
      sessionId,
    ) => sandboxConfigForSession(sessionId),
  ) {}
  ...
  resolve(
    ...
    if (!containingRoot) {
      const kind: SandboxViolationKind =
        config.resolveSymlinks && realPath !== resolved
          ? "symlink_escape"
          : "boundary_escape";
      this.deny(
        kind,
    ...
    const segments = path
      .relative(containingRoot!, realPath)
      .split(path.sep)
      .filter(Boolean);
  ...
  private resolveReal(targetPath: string): string {
    try {
      ...
      return path.resolve(
        ...
        path.relative(current, targetPath),
      );
    }
  }
