...
export class AgentSessionRuntime {
  ...
  create(input: CreateSessionRequest): AgentSession {
    ...
    const requestedLocation = (input.sessionMetadata?.workspaceLocation as WorkspaceLocation | undefined)
      ?? parentBackend?.workspaceLocation;
    ...
    if (parentBackend?.workspaceRoots) {
      backend.workspaceRoots = parentBackend.workspaceRoots.map((root) => ({
        ...
        ...(root.role === "primary" && backend.workDir
          ? { path: backend.workspaceLocation?.path ?? backend.workDir, ...(backend.workspaceLocation ? { location: backend.workspaceLocation } : {}) }
          : {}),
      }));
    }
