...
export function searchSessions(
  ...
  const sql = `WITH hits AS (
    ...
    WHERE s.archived_at IS NULL AND NOT EXISTS(SELECT 1 FROM conversation_v3_deletions d WHERE d.session_id=s.id) AND s.project_id = ? AND s.parent_session_id IS NULL AND ${hit("agent_search_sessions")}
    UNION ALL
    ...
    WHERE s.archived_at IS NULL AND NOT EXISTS(SELECT 1 FROM conversation_v3_heads h WHERE h.session_id=s.id) AND NOT EXISTS(SELECT 1 FROM conversation_v3_deletions d WHERE d.session_id=s.id) AND s.project_id = ? AND s.parent_session_id IS NULL AND ${hit("agent_search_messages")}
  ), ranked AS (
