...
class GoalModePromptStrategy extends SynaxModePromptStrategy {
  ...
  buildSection(context: SynaxModePromptContext): string | null {
    ...
    const lines = ['Session mode: goal. Persist toward the authorized objective; follow the current Work checkpoint.'];
    if (!state && (context.metadata.goalContent || context.prompt)) lines.push(`Objective: ${context.metadata.goalContent || context.prompt}`);
    ...
    lines.push('Submit completion through goal.finish or work.checkpoint with criterion evidence. Plain final text is not goal acceptance. Control tools occupy a step alone.');
    if (context.metadata.source === 'goal-dock' && context.metadata.documentId)
      lines.push(`Related Wiki document: ${context.metadata.documentId}. Keep affected documentation aligned with the authorized change.`);
...
function buildStoredPlanSection(metadata: Record<string, unknown>): string | null {
  ...
  lines.push(value.status === 'approved' ? 'This revision is approved. Mode changes do not discard its acceptance requirements.' : 'Saved, not executing. Execution requires plan.execute after explicit user intent or the plan approval interaction.');
  return lines.join('\n');
}
