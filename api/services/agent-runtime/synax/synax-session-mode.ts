...
export const SYNAX_SESSION_MODES = [
  "chat",
  "plan",
  "goal",
  "plan_node",
...
export interface SynaxSessionMetadata {
  ...
  userPrompt?: string;
  /** @deprecated Read-only compatibility with stored sessions; write userPrompt. */
  goalContent?: string;
  goalId?: string;
...
export function isSynaxProfile(profileId: string): boolean {
  return (
    profileId === SYNAX_AGENT_PROFILE_ID || profileId === LEGACY_GOAL_PROFILE_ID
  );
}
...
export function parseSynaxSessionMode(
  ...
  return (SYNAX_SESSION_MODES as readonly string[]).includes(mode)
    ? (mode as SynaxSessionMode)
    : null;
}
