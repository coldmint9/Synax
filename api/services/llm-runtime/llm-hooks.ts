import type { UsageNormalization } from "./usage.js";
...
export class LlmHookRegistry {
      ...
      try {
        ...
        if (result && typeof result === "object" && "catch" in result) {
          (result as Promise<void>).catch((err) => {
            logger.warn(
              { hookId: hook.id, eventType: event.type, err },
              "[llm-hooks] async handler failed",
            );
          });
        }
      } catch (err) {
  ...
  private matches(hook: LlmHook, event: LlmHookEvent): boolean {
    ...
    if (f.purpose && "context" in event && event.context?.purpose !== f.purpose)
      return false;
    return true;
  }
}
