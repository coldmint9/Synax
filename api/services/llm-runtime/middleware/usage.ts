...
export function applyUsageMiddleware(
  ...
  const metadata = (existing: unknown, raw: unknown, requestId: string) =>
    ({
      ...record(existing),
      synaxUsage: {
        version: 1,
    ...
    middleware: {
      ...
      wrapGenerate: async ({ doGenerate }) => {
        ...
        return {
          ...
          providerMetadata: metadata(result.providerMetadata, raw, requestId),
        };
      },
              ...
              transform(part, controller) {
                ...
                if (part.type === "finish") {
                  ...
                  controller.enqueue({
                    ...
                    usage: {
                      ...part.usage,
                      raw: whitelistRawUsage(evidence) as JSONObject,
                    },
