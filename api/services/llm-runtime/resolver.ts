...
export function resolveLlmSelection(
  ...
  // An explicit turn selection must never silently become a different model.
  if (input.modelOverride?.trim()) {
    const candidate = normalizeModelRef(
      input.modelOverride,
      resolvePreferredProviderId(input),
    );
    const resolved = tryResolveCandidate(input.catalog, input, candidate);
    if (resolved) return resolved;
    throw new Error(
      `Requested LLM model '${input.modelOverride}' is unavailable for its provider. Select a configured provider/model in Settings.`,
    );
  }
...
export function mergeProviderConfig(
  ...
  const mergedOptions = mergeObject(
...
export function resolveProviderApiFormat(input: {
  ...
  const format =
    input.apiFormat ?? toRecord(input.connection?.extra)?.apiFormat;
  if (
    format === "anthropic" ||
  ...
  if (
    ...
    isResponsesFirstModel(input.modelId, input.modelReasoning)
  ) {
...
function findModel(
  ...
  const reasoningCapable = inferReasoningCapability({
...
function isModelAllowed(
  ...
  if (config.whitelist?.length && !config.whitelist.includes(modelId))
    return false;
...
function resolveProjectModelRef(
  input: ResolveLlmSelectionInput,
): string | undefined {
...
function resolveDefaultApiModelRef(
  input: ResolveLlmSelectionInput,
): string | undefined {
  const providerId = input.globalConfig.defaultApiProviderId;
  const modelId =
    resolveModelIdFromConnection(
...
function resolveDefaultModelFromProviderDef(
  input: ResolveLlmSelectionInput,
  providerId: string,
): string | undefined {
  const provider = input.globalConfig.providers.find(
...
function isApiProvider(
  ...
  return input.globalConfig.providers.some(
...
function enabledApiProviderIds(input: ResolveLlmSelectionInput): string[] {
  return unique([
    ...
    isApiProvider(input.projectConfig.providerId, input)
      ? input.projectConfig.providerId
      : undefined,
...
export function resolveRuntimeProvider(
  ...
  const fromCatalog = input.catalog.providers.find(
    (provider) => provider.id === providerId,
  );
  const providerDef = input.globalConfig.providers.find(
    (provider) => provider.id === providerId,
  );
  ...
  if (fromCatalog) {
    if (providerDef && providerDef.models.length > 0) {
      ...
      return {
        ...fromCatalog,
        models: toRuntimeModels(providerDef, connection).map((model) => ({
          ...fromCatalog.models.find((m) => m.id === model.id),
          ...model,
          inputModalities:
...
function createConfiguredRuntimeProvider(
  provider: ProviderDef,
  connection?: ProviderConnection,
): RuntimeProvider {
  const { npm, api, env } = resolveConfiguredProviderAdapter(
    provider.id,
    connection,
  );
  return {
...
const KNOWN_PROVIDER_ADAPTERS: Record<
  string,
...
function mergeConnectionMetadata(
  ...
  return {
    providerId:
      projectConnection?.providerId ?? globalConnection?.providerId ?? "",
    ...globalConnection,
    ...projectConnection,
    extra: {
...
function resolveModelIdFromConnection(
  connection?: ProviderConnection | null,
): string | undefined {
...
function normalizeConnection(
  connection?: ProviderConnection,
): Omit<ResolvedProviderConfig, "providerId"> {
  ...
  const apiFormat: ApiFormat | undefined =
    storedApiFormat === "openai" ||
    storedApiFormat === "openai-responses" ||
    storedApiFormat === "anthropic"
      ? storedApiFormat
      : undefined;
...
function toModelOverrideMap(
  value: unknown,
): Record<string, ModelOverrideConfig> | undefined {
  const input = toRecord(value);
