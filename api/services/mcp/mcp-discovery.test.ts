...
describe('discoverLocalMcpServers', () => {
  it('discovers JSONC, OpenCode, and Codex stdio servers and deduplicates them', () => {
    ...
    write(path.join(project, '.cursor', 'mcp.json'), `{
    ...
    write(path.join(home, '.cursor', 'mcp.json'), JSON.stringify({
      mcpServers: {
        'same-command': { command: 'npx', args: ['-y', '@modelcontextprotocol/server-filesystem', '.'] },
      },
    }))
    ...
    const result = discoverLocalMcpServers(project, { homeDir: home, platform: 'linux', env: {} })

    expect(result.scannedFiles).toBe(4)
    expect(result.warnings).toEqual([])
    expect(result.servers).toHaveLength(3)
    expect(result.servers.map(item => item.server.name)).toEqual(['browser', 'filesystem', 'github'])
    expect(result.servers.find(item => item.server.name === 'filesystem')?.sources).toEqual([
      { client: 'Cursor', path: './.cursor/mcp.json', scope: 'project' },
      { client: 'Cursor', path: '~/.cursor/mcp.json', scope: 'user' },
    ])
    expect(result.servers.find(item => item.server.name === 'browser')?.server).toMatchObject({
      command: 'npx',
      args: ['-y', 'browser-mcp'],
      env: { PORT: '3333' },
      cwd: project,
    ...
    expect(result.servers.find(item => item.server.name === 'github')?.server.env).toEqual({ TOKEN: 'secret' })
  })
  ...
  it('reads only the active project section from Claude Code user settings', () => {
    ...
    write(path.join(home, '.claude.json'), JSON.stringify({
      ...
      projects: {
        ...
        [path.join(home, 'other-project')]: { mcpServers: { unrelated: { command: 'other-server' } } },
      },
    }))

    const result = discoverLocalMcpServers(project, { homeDir: home, platform: 'linux', env: {} })

    expect(result.servers.map(item => item.server.name)).toEqual(['global', 'local'])
    expect(result.servers.find(item => item.server.name === 'global')?.sources[0].scope).toBe('user')
    expect(result.servers.find(item => item.server.name === 'local')?.sources[0].scope).toBe('project')
  })
  ...
  it('discovers project Codex and Gemini configs and preserves execution context', () => {
    ...
    write(path.join(project, '.codex', 'config.toml'), `
      ...
      cwd = "tools"
    `)
    write(path.join(project, '.gemini', 'settings.json'), JSON.stringify({
      mcpServers: {
        'gemini-local': {
          command: 'node',
    ...
    expect(result.servers.map(item => item.server.name)).toEqual(['codex-local', 'gemini-local'])
    expect(result.servers.find(item => item.server.name === 'codex-local')?.server).toMatchObject({
      args: ['${workspaceFolder}/server.mjs', '${env:MCP_TOKEN}'],
      cwd: path.join(project, 'tools'),
    })
    expect(result.servers.find(item => item.server.name === 'gemini-local')?.server).toMatchObject({
      args: ['${userHome}/gemini-server.mjs'],
      cwd: project,
      env: { TOKEN: '${env:MCP_TOKEN}' },
    })
  ...
  it('reports unreadable configs without failing the remaining discovery', () => {
    ...
    expect(result.warnings[0]).toMatchObject({ client: 'Claude Code', path: './.mcp.json' })
  })
})
