...
function normalizeWorkingDirectory(
  ...
  let cwd = value.trim();
  if (fallback)
    cwd = cwd.replace(/\$\{(?:workspaceFolder|workspaceRoot)\}/g, fallback);
  cwd = cwd.replace(/\$\{userHome\}/g, homeDir);
  if (cwd === "~") cwd = homeDir;
  else if (cwd.startsWith(`~${path.sep}`))
    cwd = path.join(homeDir, cwd.slice(2));
  if (!path.isAbsolute(cwd) && !cwd.includes("${") && fallback)
    cwd = path.resolve(fallback, cwd);
  return cwd;
}
...
function normalizeServer(
  ...
  if (
    ...
    if (!record.cwd && !hasProjectBase)
      throw new Error(`Cannot resolve relative MCP executable '${command}' from a user-level configuration without an explicit cwd.`);
    const absolute =
      cwd && path.isAbsolute(cwd) ? path.resolve(cwd, command) : undefined;
    let exists = false;
    try {
      exists = !!absolute && fs.statSync(absolute).isFile();
    } catch {
      /* unavailable */
    }
    if (!exists)
      throw new Error(
        `Cannot resolve relative MCP executable '${command}' from cwd '${cwd ?? "(unset)"}'. Configure an absolute command or the original executable's cwd.`,
      );
    command = absolute!;
  }
  return {
...
function serverFingerprint(
  server: Omit<McpServerConfig, "id" | "name" | "enabled">,
): string {
...
function entryGroups(
  ...
  for (const [projectPath, projectValue] of Object.entries(projects)) {
    ...
    if (projectEntries)
      groups.push({ entries: projectEntries, scope: "project" });
  }
  return groups;
}
...
function configLocations(
  ...
  const add = (
    ...
    locations.push({
      ...
      format,
      scope,
      ...(scope === "project" ? { workspaceDirectory: projectDirectory } : {}),
    });
  };
  ...
  if (projectDirectory) {
    ...
    add(
      "Cursor",
      path.join(projectDirectory, ".cursor", "mcp.json"),
      "standard",
      "project",
    );
    add(
      "VS Code",
      path.join(projectDirectory, ".vscode", "mcp.json"),
      "vscode",
      "project",
    );
    add(
      "Windsurf",
      path.join(projectDirectory, ".windsurf", "mcp_config.json"),
      "standard",
      "project",
    );
    add(
      "Roo Code",
      path.join(projectDirectory, ".roo", "mcp.json"),
      "standard",
      "project",
    );
    add(
      "Kilo Code",
      path.join(projectDirectory, ".kilocode", "mcp.json"),
      "standard",
      "project",
    );
    add(
    ...
    add(
      "Gemini CLI",
      path.join(projectDirectory, ".gemini", "settings.json"),
      "standard",
      "project",
    );
  }
  ...
  add(
    "Codex",
    path.join(env.CODEX_HOME || path.join(homeDir, ".codex"), "config.toml"),
    "codex",
    "user",
  );
  add(
  ...
  for (const directory of options.extraDirectories ?? []) {
    for (const location of configLocations(directory, {
      ...options,
      extraDirectories: [],
    })) {
  ...
  for (const location of locations)
    unique.set(
      `${location.format}\0${path.resolve(location.filePath)}`,
      location,
    );
...
export function discoverLocalMcpServers(
  ...
  for (const location of configLocations(projectDirectory, options)) {
    ...
    const report: McpDiscoveryResult["locations"][number] = {
    ...
    try {
      ...
      if (stat.size > MAX_CONFIG_BYTES)
        throw new Error("Configuration file is larger than 1 MB");
      report.status = "found";
      scannedFiles += 1;
      ...
      for (const group of entryGroups(location, parsed, projectDirectory)) {
        for (const [name, value] of Object.entries(group.entries)) {
          let normalized: ReturnType<typeof normalizeServer>;
          try {
            normalized = normalizeServer(
              name,
              value,
              location.workspaceDirectory ?? projectDirectory,
              homeDir,
              group.scope === "project" || Boolean(location.workspaceDirectory),
            );
          } catch (error) {
            unsupported.push({
              client: location.client,
  ...
  return {
    servers: [...discovered.values()].sort((a, b) =>
