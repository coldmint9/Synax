...
export function projectSourceLocation(project: WorkspaceProject): WorkspaceLocation | undefined {
  const source = project.source;
  if (!source) return undefined;
  if (source.kind === 'wsl' && source.distribution && source.path) {
    return { kind: 'wsl', distribution: source.distribution, path: source.path };
  }
  if (source.localPath) return hostWorkspaceLocation(source.localPath);
  return undefined;
}

export function projectReferenceLocation(reference: ProjectReference): WorkspaceLocation | undefined {
  return reference.location ?? (reference.localPath ? hostWorkspaceLocation(reference.localPath) : undefined);
}

export function workspaceRootLocation(root: ProjectWorkspaceRoot): WorkspaceLocation {
...
export function workspaceRootHostPath(root: ProjectWorkspaceRoot): string {
  return root.hostPath ?? workspaceLocationHostPath(workspaceRootLocation(root));
}

function withInternalHostPath(root: ProjectWorkspaceRoot): ProjectWorkspaceRoot {
  const location = workspaceRootLocation(root);
  if (location.kind === 'host') {
    Object.defineProperty(root, 'location', { configurable: true, enumerable: false, value: location });
  }
...
export function projectWorkspaceRoots(project: WorkspaceProject): ProjectWorkspaceRoot[] {
  const roots: Array<Omit<ProjectWorkspaceRoot, 'status' | 'path'> & { location: WorkspaceLocation }> = [];
  const primary = projectSourceLocation(project);
  if (primary) roots.push({ id: project.id, name: project.primaryName ?? project.name ?? project.id, location: primary, role: 'primary' });
  for (const reference of project.references ?? []) {
    const location = projectReferenceLocation(reference);
    if (location) roots.push({ id: reference.id, name: reference.name, location, role: 'reference' });
  }
  return roots.map(input => {
    try {
      const location = canonicalizeWorkspaceLocationSync(input.location);
      return withInternalHostPath({ ...input, location, path: location.path, status: 'available' });
    } catch {
      return withInternalHostPath({ ...input, path: input.location.path, status: 'missing' });
    }
  });
}
...
export async function validateProjectReferenceLocation(
  ...
  const primary = projectSourceLocation(project);
  if (!primary) throw new Error('The main project must have a local workspace before adding references.');
  const existingLocations = projectWorkspaceRoots(project).map(workspaceRootLocation);
  ...
  for (const root of projectWorkspaceRoots(project)) {
    ...
    if (locationContains(existing, candidate) || locationContains(candidate, existing)) {
...
export function validateProjectReference(project: WorkspaceProject, input: string): string {
  const primary = projectSourceLocation(project);
  if (!primary || primary.kind !== 'host') throw new Error('The main project must have a local workspace before adding references.');
  canonicalWorkspaceDirectory(primary.path);
  const candidate = canonicalWorkspaceDirectory(input);
  for (const root of projectWorkspaceRoots(project)) {
    const location = workspaceRootLocation(root);
    if (location.kind !== 'host') throw new Error('Workspace roots must use the same environment.');
