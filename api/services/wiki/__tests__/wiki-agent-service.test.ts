...
vi.mock("../wiki-model.js", () => ({
  getWikiWorkflowModel: () => "openai/wiki-model",
}));

vi.mock("../../llm-runtime/gateway.js", () => ({
  generateGatewayObject: (...args: unknown[]) =>
    mockGenerateGatewayObject(...args),
}));
...
describe("wikiAgentService.generateWiki", () => {
  ...
  it("returns markdown documents from the model", async () => {
    ...
    const result = await wikiAgentService.generateWiki(makeScan(), {
      projectId: "proj-1",
    });
    expect(result.documents).toHaveLength(1);
    expect(result.documents[0].markdown).toContain("# Overview");
    expect(mockGenerateGatewayObject).toHaveBeenCalledWith(
      expect.objectContaining({ model: "openai/wiki-model" }),
      expect.anything(),
    );
  });
