...
import { buildSessionPrompt } from "../agent-runtime/session-prompt.js";
import { PLAN_NODE_PERMISSION_OVERRIDES } from "./wiki-plan-permissions.js";
import {
  getPlan,
  listPlanNodes,
  listGoals,
...
} from "../agent-runtime/tools/patch-format.js";
...
export function subscribePlanExecution(
  planId: string,
...
async function runExecutionLoop(
  ...
  while (true) {
    ...
    const reviewNode = (await listPlanNodes(planId)).find(
      (n) => n.status === "review",
    );
    if (reviewNode) {
...
async function executeNode(
  ...
  for await (const chunk of stream) {
    if (chunk.type === "message_delta" && chunk.delta)
      logParts.push(chunk.delta);
    if (chunk.type === "thought_delta" && chunk.delta)
      logParts.push(chunk.delta);
    if (chunk.type === "run_failed")
      throw new Error(chunk.error ?? "Agent run failed");
  }
...
export async function acceptPlanNode(
  planId: string,
  nodeId: string,
  workDir: string,
): Promise<void> {
  ...
  if (!artifact || artifact.status !== "generated")
    throw new Error("No generated artifact to accept");
  ...
  if (pending) {
    ...
    const allReviewed = nodes.every(
      (n) => n.status === "committed" || n.status === "accepted",
    );
...
export async function redoPlanNode(
  ...
  if (prev)
    await updateArtifact(prev.id, {
      status: "discarded",
      redoFeedback: feedback,
      redoCount: (prev.redoCount ?? 0) + 1,
    });
