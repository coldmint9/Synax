...
export function workspaceLocationHostPath(location: WorkspaceLocation): string {
  ...
  const suffix =
    linuxPath === "/" ? "" : linuxPath.split("/").filter(Boolean).join("\\");
  return suffix
    ? `${wslUncRoot(location.distribution)}\\${suffix}`
    : wslUncRoot(location.distribution);
}

export function parseWslUncPath(
  input: string,
): Extract<WorkspaceLocation, { kind: "wsl" }> | null {
  ...
  return {
    ...
    path: path.posix.normalize(linuxPath),
  };
}
...
export async function canonicalizeWorkspaceLocation(
  location: WorkspaceLocation,
): Promise<WorkspaceLocation> {
  if (location.kind === "host")
    return { kind: "host", path: canonicalHostDirectory(location.path) };
  return {
    kind: "wsl",
    distribution: location.distribution,
    path: await canonicalizeWslPath(location.distribution, location.path),
  };
}

export function canonicalizeWorkspaceLocationSync(
  location: WorkspaceLocation,
): WorkspaceLocation {
  if (location.kind === "host")
    return { kind: "host", path: canonicalHostDirectory(location.path) };
  ...
  try {
    ...
    throw new Error(
      `Workspace directory is unavailable: ${workspaceLocationDisplay(location)}`,
    );
  }
...
export function locationKey(location: WorkspaceLocation): string {
  if (location.kind === "wsl")
    return `wsl:${location.distribution.toLocaleLowerCase()}:${path.posix.normalize(location.path)}`;
...
export function sameExecutionEnvironment(
  ...
  return (
    left.kind === "host" ||
    left.distribution.toLocaleLowerCase() ===
      (
        right as Extract<WorkspaceLocation, { kind: "wsl" }>
      ).distribution.toLocaleLowerCase()
  );
}

export function assertCompatibleLocations(
  locations: WorkspaceLocation[],
): void {
  if (locations.length < 2) return;
  const first = locations[0];
  for (const location of locations.slice(1)) {
    if (first.kind !== location.kind)
      throw new Error("Workspace roots must use the same environment.");
    if (
      first.kind === "wsl" &&
      location.kind === "wsl" &&
      first.distribution.toLocaleLowerCase() !==
        location.distribution.toLocaleLowerCase()
    ) {
...
export function workspaceLocationRelative(
  root: WorkspaceLocation,
  target: WorkspaceLocation,
): string {
  if (!sameExecutionEnvironment(root, target))
    throw new Error(
      "Workspace locations use different execution environments.",
    );
  return (root.kind === "wsl" ? path.posix : path).relative(
    root.path,
    target.path,
  );
}
...
export function resolveWorkspaceLocation(
  ...
  if (root.kind === "wsl")
    return {
      ...root,
      path: path.posix.resolve(root.path, input.replace(/\\/g, "/")),
    };
