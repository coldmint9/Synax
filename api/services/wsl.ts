...
export class WslError extends Error {
  constructor(message: string, readonly code = 'WSL_UNAVAILABLE') {
    super(message);
    this.name = 'WslError';
  }
}
...
export function decodeWslOutput(output: Buffer | string): string {
  ...
  const hasUtf16Bom = output.length >= 2 && output[0] === 0xff && output[1] === 0xfe;
  const nulHeavy = output.subarray(0, Math.min(output.length, 128)).filter(byte => byte === 0).length > 4;
  return output.toString(hasUtf16Bom || nulHeavy ? 'utf16le' : 'utf8').replace(/^\ufeff/, '');
}
...
function execWsl(args: string[], options: { timeout?: number; env?: NodeJS.ProcessEnv } = {}): Promise<{ stdout: string; stderr: string }> {
  return new Promise((resolve, reject) => {
    execFile(process.env.SYNAX_WSL_EXE || 'wsl.exe', args, {
      windowsHide: true,
      ...
      if (error) {
        reject(new WslError(decodedStderr.trim() || error.message, (error as NodeJS.ErrnoException).code || 'WSL_COMMAND_FAILED'));
        return;
      }
...
export async function listWslDistributions(options: { platform?: NodeJS.Platform } = {}): Promise<WslDistribution[]> {
...
export function assertWslDistributionName(name: string): string {
  ...
  if (!value || value.includes('\0') || /[\\/\r\n]/.test(value)) {
    throw new WslError('WSL distribution name is invalid.', 'WSL_DISTRIBUTION_INVALID');
  }
  return value;
}

export async function assertWslDistribution(name: string): Promise<WslDistribution> {
  name = assertWslDistributionName(name);
  const found = (await listWslDistributions()).find(item => item.name.toLocaleLowerCase() === name.toLocaleLowerCase());
  if (!found) throw new WslError(`WSL2 distribution is not installed: ${name}`, 'WSL_DISTRIBUTION_MISSING');
  return found;
}
...
const WSL_ENV_INTERNAL = new Set([
  'AGENT_SESSION_INIT', 'WIKI_JOB_INIT', 'SYNAX_AGENT_SESSION_CHILD',
  'SYNAX_WIKI_JOB_CHILD', 'SYNAX_RUNTIME_HOST_ID', 'SYNAX_RUNTIME_DATA_ROOT',
]);

/** Keep the Windows launcher environment, but forward only explicit overrides through WSLENV. */
export function wslLauncherEnvironment(overrides?: NodeJS.ProcessEnv): NodeJS.ProcessEnv {
  const env: NodeJS.ProcessEnv = { ...process.env };
  const forwarded: string[] = [];
  for (const [key, value] of Object.entries(overrides ?? {})) {
    if (value === undefined || WSL_ENV_BLOCKED.has(key.toUpperCase()) || WSL_ENV_INTERNAL.has(key)) continue;
...
export function wslCommandSpec(
  ...
  const target = options.shell ? ['/bin/sh', '-c', command] : [command, ...args];
  if (options.ownerId) {
    return {
      command: 'wsl.exe',
      args: [...base, '/usr/bin/env', `SYNAX_PROCESS_OWNER=${options.ownerId}`, '/usr/bin/setsid', ...target],
    };
  }
...
export function assertLinuxAbsolutePath(input: string): string {
  ...
  if (!value || value.includes('\0') || !path.posix.isAbsolute(value)) {
    throw new WslError('WSL workspace path must be an absolute Linux path.', 'WSL_PATH_INVALID');
  }
  return path.posix.normalize(value);
}

export async function canonicalizeWslPath(distribution: string, input: string): Promise<string> {
  ...
  if (!canonical || !path.posix.isAbsolute(canonical)) {
    throw new WslError(`WSL workspace directory is unavailable: ${candidate}`, 'WSL_PATH_MISSING');
  }
  return path.posix.normalize(canonical);
}

export async function wslHomeDirectory(distribution: string): Promise<string> {
  const { stdout } = await runWsl(distribution, '/', '/bin/sh', ['-c', 'printf %s "$HOME"']);
...
export async function findWslOwnedProcess(distribution: string, ownerId: string): Promise<WslOwnedProcess | null> {
  try {
    const { stdout } = await runWsl(distribution, '/', '/bin/sh', ['-c', OWNER_LOOKUP_SCRIPT, 'synax', ownerId], { timeout: 4_000 });
...
exit 1`;
  try {
    await runWsl(distribution, '/', '/bin/sh', ['-c', script, 'synax', String(found.pgid)], { timeout: 5_000 });
    return true;
  } catch { return false; }
}
