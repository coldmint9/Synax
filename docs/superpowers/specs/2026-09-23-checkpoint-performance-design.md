...
状态：方向已批准并继续推进实现；具体预算作为初始工程验收目标，尚无达标声明。
...
## 11. 2026-09-23 基于实测的 GC / 引用表示修订

首轮核心压测发现引用表和反向索引约占数据库 45%。对不可变对象 DAG，改为 32 字节二进制标识、有界紧凑邻接列表和持久化引用计数，利用零引用部分索引分批回收；替代第 4.7 节对此 DAG 的 mark/sweep 初始选择。新对象必须先验证/引用已存在子对象，再插入自身，因此不能形成引用环。头指针和显式 pin 在同一事务增减强引用；ownership 证明不是强引用，并在 GC 中分页清除，不能靠无界 FK 级联删除。

每轮仍遵循对象、引用、字节、时间和元数据行数预算。不可变引用不能原地更新；未来引入可变引用时必须重新证明无环性，不能继续沿用该回收算法。真实 v2 迁移与运行时接入仍未完成；非空的未发布 v3 原型数据不得被自动重置。
