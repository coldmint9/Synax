...
export interface UiFile { path: string; sha256: string; size: number }
export interface UiArtifact { name: string; sha256: string; size: number }
...
function validArtifact(value: unknown, name: string): value is UiArtifact {
  ...
  return !!item && item.name === name && hex.test(item.sha256) &&
    Number.isSafeInteger(item.size) && item.size > 0 && item.size <= UI_ARCHIVE_LIMIT;
}

export function validateManifest(value: unknown): UiManifest {
  const data = value as UiManifest;
  if (!data || data.format !== 1 || !version.test(data.version) ||
      !version.test(data.appVersion) || !Array.isArray(data.files) ||
      !data.files.length || data.files.length > UI_FILES_LIMIT ||
  ...
  for (const entry of data.files) {
    if (!entry || !validUiPath(entry.path) || entry.path === "manifest.json" || !hex.test(entry.sha256) ||
        !Number.isSafeInteger(entry.size) || entry.size < 0 ||
        entry.size > UI_CONTENT_LIMIT || paths.has(entry.path)) {
  ...
  if (data.delta !== undefined) {
    ...
    if (!validArtifact(delta, "ui-delta.json.gz") ||
        !version.test(delta.baseVersion) || !Array.isArray(delta.paths) ||
        delta.paths.length > UI_FILES_LIMIT || new Set(delta.paths).size !== delta.paths.length ||
...
export function decodeUiArchive(archive: Buffer, expected: UiFile[]): Map<string, Buffer> {
  if (archive.length > UI_ARCHIVE_LIMIT) throw new Error("UI archive exceeds size limit");
  const value = JSON.parse(gunzipSync(archive, { maxOutputLength: UI_JSON_LIMIT }).toString("utf8"));
  if (value?.format !== 1 || !Array.isArray(value.files) || value.files.length !== expected.length) {
    throw new Error("Invalid UI archive");
  }
  const listed = new Map(expected.map((entry) => [entry.path, entry]));
  const decoded = new Map<string, Buffer>();
  for (const item of value.files) {
    if (!Array.isArray(item) || item.length !== 2 || !validUiPath(item[0]) ||
        typeof item[1] !== "string" || !/^(?:[A-Za-z0-9+/]{4})*(?:[A-Za-z0-9+/]{2}==|[A-Za-z0-9+/]{3}=)?$/.test(item[1])) {
      throw new Error("Invalid UI archive entry");
    }
    const entry = listed.get(item[0]);
    if (!entry || decoded.has(item[0])) throw new Error("Unexpected UI archive entry");
