...
async function release(
  ...
  const assets = await createUiReleaseArtifacts(
    dist,
    version,
    "0.1.2",
    previous?.manifest,
  ...
  return {
    ...
    assets: [
      assets.manifest.full,
      ...(assets.manifest.delta ? [assets.manifest.delta] : []),
    ].map(({ name }) => ({
      name,
      browser_download_url: "https://github.com/test",
    })),
  };
}
...
describe("UI snapshot store", () => {
  it("stages a first full update, applies on restart, then uses an adjacent delta", async () => {
    ...
    expect(
      await fs.readFile(path.join(restarted.root, "index.html"), "utf8"),
    ).toBe("first");
    ...
    expect(
      await fs.readFile(path.join(upgraded.root, "index.html"), "utf8"),
    ).toBe("second");
    expect(
      await fs.readFile(path.join(upgraded.root, "unchanged.txt"), "utf8"),
    ).toBe("retain");
    await expect(
      fs.access(path.join(upgraded.root, "obsolete.txt")),
    ).rejects.toThrow();
  ...
  it("falls back to full UI when the cached delta base is corrupted", async () => {
    ...
    await upgraded.initialize();
    expect(
      await fs.readFile(path.join(upgraded.root, "retain.txt"), "utf8"),
    ).toBe("valid");
  ...
  it("recovers to bundled UI when the cached snapshot has been damaged", async () => {
    ...
    const current = store();
    await current.initialize();
    await current.prepare(target);
    const upgraded = store();
    await upgraded.initialize();
    await upgraded.markHealthy();
