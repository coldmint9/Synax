...
import { decodeUiArchive, sha256, validateManifest, type UiFile } from "./ui-update-format.js";
...
const initialState = (): UiState => ({ active: null, pending: null, previous: null, awaitingHealth: false, rejected: [] });
...
export class UiUpdateStore {
  ...
  constructor(private readonly directory: string, private readonly bundledRoot: string, private readonly appVersion: string) {
    this.currentRoot = bundledRoot;
  }

  get root(): string { return this.currentRoot; }
  get needsHealthCheck(): boolean { return this.state.awaitingHealth; }
  get currentVersion(): string | null { return this.state.active; }
  get pendingVersion(): string | null { return this.state.pending; }
  get rejected(): string[] { return this.state.rejected; }
  private snapshot(version: string): string { return path.join(this.directory, `ui-${version}`); }
  private async save(): Promise<void> { await atomicJson(path.join(this.directory, "state.json"), this.state); }

  private async cleanCache(): Promise<void> {
    const keep = new Set([this.state.active, this.state.pending, this.state.previous].filter(Boolean));
    for (const entry of await fs.readdir(this.directory, { withFileTypes: true })) {
      if (!entry.isDirectory()) continue;
      if (/^\.stage-[0-9a-f-]{36}$/.test(entry.name) ||
          /^ui-\d+\.\d+\.\d+$/.test(entry.name) && !keep.has(entry.name.slice(3))) {
  ...
  private async loadValid(version: string | null): Promise<string | null> {
    ...
    try {
      ...
      if (manifest.version !== version || manifest.appVersion !== this.appVersion) return null;
  ...
  async initialize(): Promise<void> {
    ...
    try {
      const input = await readJson(path.join(this.directory, "state.json")) as UiState;
      if (!input || !Array.isArray(input.rejected) ||
          ![input.active, input.pending, input.previous].every((v) => v === null || typeof v === "string") ||
          typeof input.awaitingHealth !== "boolean") throw new Error("Invalid UI state");
      this.state = { ...input, rejected: input.rejected.filter((v) => /^\d+\.\d+\.\d+$/.test(v)).slice(-20) };
    ...
    await this.save();
    await this.cleanCache().catch((error) => console.warn("[ui-update] cache cleanup failed", error));
  }
  ...
  async rollback(): Promise<boolean> {
    ...
    this.state.previous = null;
    this.state.awaitingHealth = false;
    this.currentRoot = await this.loadValid(this.state.active) ?? this.bundledRoot;
  ...
  async prepare(release: UiRelease): Promise<"delta" | "full"> {
    ...
    if (manifest.appVersion !== this.appVersion || this.state.rejected.includes(manifest.version)) {
      throw new Error("UI release is not compatible with this app");
    }
    const delta = manifest.delta?.baseVersion === this.state.active &&
      release.assets.some((asset) => asset.name === manifest.delta?.name) ? manifest.delta : undefined;
    ...
    try {
      ...
      const expected = delta ? manifest.files.filter((entry) => delta.paths.includes(entry.path)) : manifest.files;
      incoming = decodeUiArchive(await fetchUiArtifact(release, artifact), expected);
    } catch (error) {
      if (!delta) throw error;
      console.warn("[ui-update] delta unavailable; fetching full UI", error);
      mode = "full";
      incoming = decodeUiArchive(await fetchUiArtifact(release, manifest.full), manifest.files);
    }
    ...
    try {
      for (const entry of manifest.files) {
        ...
        const contents = incoming.get(entry.path) ??
          (delta && mode === "delta" && this.state.active
            ? await fs.readFile(path.join(this.snapshot(this.state.active), ...entry.path.split("/")))
            : undefined);
        if (!contents || contents.length !== entry.size || sha256(contents) !== entry.sha256) {
      ...
      if (mode === "delta" && delta &&
          (error instanceof Error && (/checksum mismatch/.test(error.message) || "code" in error && error.code === "ENOENT"))) {
        console.warn("[ui-update] delta base invalid; fetching full UI", error);
        return this.prepare({ ...release, manifest: { ...manifest, delta: undefined } });
      }
      throw error;
