...
describe("UI release packaging", () => {
  it("generates a full snapshot and only changed/new files in the delta", async () => {
    ...
    const next = await createUiReleaseArtifacts(
      root,
      "1.0.1",
      "0.1.2",
      first.manifest,
    ...
    expect(next.manifest.delta?.paths).toEqual([
      "assets/added.js",
      "index.html",
    ]);
    expect(
      decodeUiArchive(
        next.delta!,
        next.manifest.files.filter((entry) =>
          next.manifest.delta!.paths.includes(entry.path),
        ),
      ).size,
    ).toBe(2);
