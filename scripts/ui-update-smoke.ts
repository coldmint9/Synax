...
const packaged = path.resolve(
  "out",
  `Synax-${process.platform}-${process.arch}`,
);
...
try {
  ...
  await writeFile(
    index,
    html.replace("</head>", "<!-- ui-update-smoke-marker --></head>"),
  );
  const { manifest } = await createUiReleaseArtifacts(
    snapshot,
    "1.0.0",
    version,
  );
  ...
  assert.equal(
    await page.evaluate(() =>
      document.documentElement.innerHTML.includes("ui-update-smoke-marker"),
    ),
    true,
  );
  ...
  for (let i = 0; i < 100; i++) {
    healthy =
      JSON.parse(await readFile(path.join(updates, "state.json"), "utf8"))
        .awaitingHealth === false;
    if (healthy) break;
  ...
  assert.equal(
    healthy,
    true,
    "updated UI should acknowledge its first healthy render",
  );
  await desktop.close();
  desktop = undefined;
  ...
  const state = JSON.parse(
    await readFile(path.join(updates, "state.json"), "utf8"),
  );
  assert.equal(state.active, null);
  assert(state.rejected.includes("1.0.0"));
  console.log(
    `UI update smoke passed: ${process.platform}/${process.arch}; activation, API and bundled fallback.`,
  );
} catch (error) {
  await mkdir("out", { recursive: true });
  await writeFile(
    path.join("out", "ui-update-smoke.log"),
    error instanceof Error ? (error.stack ?? error.message) : String(error),
  );
  await desktop
    ?.windows()[0]
    ?.screenshot({ path: path.join("out", "ui-update-smoke.png") })
    .catch(() => {});
  throw error;
} finally {
