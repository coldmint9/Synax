...
} from "../react/state/notificationStore";
import {
...
export function handleError(err: unknown): AppError {
  // AbortError is normal control flow for superseded requests. It must not
  // change global reachability or produce a user-facing notification.
  if (isAbortError(err)) {
    return new AppError(err.message || "Request aborted", {
      level: "business",
...
function isNetworkAppError(err: AppError): boolean {
  return (
    err.level === "system" &&
    (err.statusCode === 0 || err.statusCode === undefined)
  );
}
...
const LLM_CONFIG_ERROR_CODES = new Set([
  "LLM_PROVIDER_NOT_CONFIGURED",
  "API_KEY_MISSING",
]);
...
function routeError(err: AppError): void {
  ...
  const isMissingRuntimeResource =
    err.code === "NOT_FOUND" &&
  ...
  useNotificationStore.getState().push({
    ...
    message,
    ...(err.code &&
      LLM_CONFIG_ERROR_CODES.has(err.code) && {
        action: {
          label: "前往配置",
          onClick: () => {
