...
export function ProjectCard({
  ...
  function relativeTime(iso: string | null): string {
    ...
    if (diff < 3_600_000)
      return t("cardMinutesAgo", { count: Math.floor(diff / 60_000) });
    if (diff < 86_400_000)
      return t("cardHoursAgo", { count: Math.floor(diff / 3_600_000) });
      ...
      <div className="flex items-center justify-between gap-2">
        ...
        <div className="flex shrink-0 items-center gap-1">
          {project.importState && project.importState !== "ready" && (
            <span
              className="import-state-badge"
              data-state={project.importState}
            >
              {project.importState === "syncing"
                ? t("cardSyncing")
      ...
      {project.source && project.source.kind !== "scratch" && (
        <div className="flex items-center gap-1.5 text-[10px] text-muted-foreground/60">
          <span
            ...
            title={formatProjectPath(project)}
          >
