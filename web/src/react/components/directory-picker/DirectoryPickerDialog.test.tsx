...
describe('DirectoryPickerDialog', () => {
  ...
  it('lists the initial directory on open and returns the chosen absolute path', async () => {
    ...
    expect(screen.queryByRole('checkbox', { name: '选择 work' })).not.toBeInTheDocument()
    expect(screen.getByRole('dialog').style).toMatchObject({ width: '42rem', height: '37.5rem', maxWidth: 'calc(100vw - 2rem)', maxHeight: 'calc(100dvh - 2rem)' })
    expect(screen.getByRole('region', { name: '选择项目目录' })).toHaveClass('overflow-y-auto')
  ...
  it('descends into an entry and confirms the new path', async () => {
    ...
    await waitFor(() => expect(screen.getByRole('textbox', { name: '目录路径' })).toHaveValue('/home/dev/work'))
  ...
  it('keeps selections across navigation and filtering and toggles the current directory without duplicates', async () => {
    vi.mocked(listRemoteDirectories).mockImplementation(async path => path === '/home/dev/work'
      ? listing({ path, name: 'work', parent: '/home/dev', entries: [{ name: 'package', path: `${path}/package`, hidden: false }] })
      : listing())
    const onSelect = vi.fn()
    const onSelectMultiple = vi.fn()
    render(<DirectoryPickerDialog open multiple onClose={vi.fn()} onSelect={onSelect} onSelectMultiple={onSelectMultiple} labels={{ confirm: '导入目录' }} />)
    ...
    fireEvent.click(await screen.findByRole('checkbox', { name: '选择 package' }))
    ...
    expect(await screen.findByRole('checkbox', { name: '选择 package' })).toBeChecked()
    expect(listRemoteDirectories).toHaveBeenLastCalledWith('/home/dev/work', expect.objectContaining({ showIgnored: true }))

    fireEvent.click(screen.getByRole('button', { name: '主目录' }))
    expect(await screen.findByRole('checkbox', { name: '选择 work' })).toBeChecked()
  ...
  it('keeps selected directories available for confirmation after a navigation error', async () => {
    vi.mocked(listRemoteDirectories).mockResolvedValueOnce(listing()).mockRejectedValueOnce(new Error('Permission denied'))
    const onSelectMultiple = vi.fn()
    render(<DirectoryPickerDialog open multiple onClose={vi.fn()} onSelect={vi.fn()} onSelectMultiple={onSelectMultiple} />)
    fireEvent.click(await screen.findByRole('checkbox', { name: '选择 work' }))
    fireEvent.click(screen.getByRole('button', { name: 'work' }))
    ...
    expect(onSelectMultiple).toHaveBeenCalledExactlyOnceWith([{ path: '/home/dev/work', name: 'work' }])
  })
  ...
  it('clears selections, filters and navigation on cancel and reopen, and restores focus', async () => {
    ...
    function Harness() {
      ...
      return <>
        ...
        <DirectoryPickerDialog open={open} multiple onClose={() => setOpen(false)} onSelect={vi.fn()} onSelectMultiple={onSelectMultiple} />
      </>
    }
    ...
    await user.click(trigger)
    expect(await screen.findByRole('checkbox', { name: '选择 work' })).not.toBeChecked()
    expect(screen.getByRole('checkbox', { name: '显示隐藏目录' })).not.toBeChecked()
    expect(screen.getByRole('textbox', { name: '目录路径' })).toHaveValue('/home/dev')
    expect(screen.getByRole('textbox', { name: '目录路径' })).toHaveFocus()
    expect(screen.getByRole('button', { name: '添加所选项目 (0)' })).toBeDisabled()
  ...
  it.each(['resolve', 'reject'] as const)('ignores a stale listing %s after closing and reopening', async outcome => {
    const old = deferred<RemoteDirectoryListing>()
    vi.mocked(listRemoteDirectories).mockReturnValueOnce(old.promise).mockResolvedValueOnce(listing({ path: '/fresh', name: 'fresh', entries: [] }))
    const props = { onClose: vi.fn(), onSelect: vi.fn() }
    const view = render(<DirectoryPickerDialog open initialPath="/old" {...props} />)
    const signal = vi.mocked(listRemoteDirectories).mock.calls[0][1]?.signal
    view.rerender(<DirectoryPickerDialog open={false} initialPath="/old" {...props} />)
    expect(signal?.aborted).toBe(true)
    view.rerender(<DirectoryPickerDialog open initialPath="/fresh" {...props} />)
    await waitFor(() => expect(screen.getByRole('textbox', { name: '目录路径' })).toHaveValue('/fresh'))
    await act(async () => {
      if (outcome === 'resolve') old.resolve(listing())
      else old.reject(new Error('old failure'))
    })

    expect(screen.getByRole('textbox', { name: '目录路径' })).toHaveValue('/fresh')
    expect(screen.queryByRole('alert')).not.toBeInTheDocument()
    fireEvent.click(screen.getByRole('button', { name: '选择此目录' }))
    expect(props.onSelect).toHaveBeenCalledExactlyOnceWith({ path: '/fresh', name: 'fresh' })
  })
  ...
  it('ignores an older navigation response even if the transport does not honor abort', async () => {
    ...
    vi.mocked(listRemoteDirectories).mockResolvedValueOnce(listing()).mockReturnValueOnce(slow.promise)
      .mockResolvedValueOnce(listing({ path: '/fast', name: 'fast', entries: [] }))
    ...
    expect(signal?.aborted).toBe(true)
    await act(async () => { slow.resolve(listing({ path: '/slow', name: 'slow' })) })
