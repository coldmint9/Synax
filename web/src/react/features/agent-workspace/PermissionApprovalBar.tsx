...
function PermissionItem({
  ...
  > = {
    ...
    external_execution: {
      labelKey: "permExternalExecution",
      tone: "caution",
      icon: Zap,
    },
    high_risk: {
      labelKey: "permHighRisk",
      tone: "danger",
      icon: AlertTriangle,
    },
  };
  const config =
    CATEGORY_CONFIG[permission.coarseCategory] ?? CATEGORY_CONFIG.high_risk;
  ...
  return (
    <div className="flex items-center gap-3 px-4 py-2.5 border-t border-border/40 bg-card/80 backdrop-blur-sm animate-in slide-in-from-bottom-2 duration-200">
      <div className="flex items-center gap-2 flex-1 min-w-0">
        <span
          className={iconBadgeClass(config.tone, "rounded-md px-1.5 py-0.5")}
