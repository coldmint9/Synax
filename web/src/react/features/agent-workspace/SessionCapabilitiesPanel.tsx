...
function ToolsCard({ tools }: { tools: SessionCapabilities["tools"] }) {
  const visibleIds = useMemo(
    () => new Set(tools.visible.map((tool) => tool.id)),
    [tools.visible],
  );
  ...
  return (
    <Accordion
      className="gap-0 border-b border-border/40 px-0"
      defaultExpandedKeys={[]}
    >
      <Accordion.Item
        id="tools"
              ...
              {sorted.map((tool) => {
                ...
                return (
                  ...
                  >
                    {visible ? (
                      <span
                        className={`mt-0.5 shrink-0 font-mono text-[8px] uppercase ${CATEGORY_COLOR[tool.category] ?? "text-muted-foreground/60"}`}
                      >
...
function SkillsCard({ skills }: { skills: SessionCapabilities["skills"] }) {
  const activeIds = useMemo(
    () => new Set(skills.active.map((skill) => skill.id)),
    [skills.active],
  );

  if (skills.active.length === 0 && skills.candidates.length === 0) return null;

  const inactiveCandidates = skills.candidates.filter(
    (skill) => !activeIds.has(skill.id),
  );
  ...
  return (
    ...
    >
      <div className="flex items-center gap-1.5">
        ...
        />
        <span
          className={`truncate text-[10px] ${active ? "font-medium text-foreground/90" : "text-foreground/70"}`}
        >
          {skill.label}
        </span>
      </div>
    ...
    >
      ...
      >
        <Accordion.Trigger className="flex w-full items-center gap-1 px-2 py-2 text-left">
          ...
          <span className="ml-auto text-[9px] text-muted-foreground/60">
            {active > 0
              ? `${servers.length} 个已挂载 · ${toolCount} tools`
              : "未挂载"}
          </span>
          <Accordion.Indicator className="text-muted-foreground/50 [&>svg]:size-3" />
        </Accordion.Trigger>
...
export function SessionCapabilitiesPanel({
  capabilities,
}: {
    ...
    <>
      {capabilities.backend && capabilities.backend.kind !== "native" && (
        <p className="border-b border-border/40 px-2 py-2 text-[10px] text-muted-foreground">
          {capabilities.backend.label}: tools and configuration are managed by
          this backend. Synax Native Skills/MCP are not injected.
        </p>
      )}
