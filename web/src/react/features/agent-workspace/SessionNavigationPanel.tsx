...
function scrollToEntry(scrollRoot: HTMLElement | null, entryId: string) {
  ...
  window.setTimeout(() => {
    if (Math.abs(target.getBoundingClientRect().height - reservedHeight) <= 1)
      return;
    target.scrollIntoView({ behavior: "smooth", block: "center" });
  }, 140);
}
...
function stripLabelStem(title: string, raw: string): string {
  ...
  let trimmed = raw.startsWith(stem)
    ? raw.slice(stem.length).trim()
    : raw.trim();
...
export function turnPreview(
  ...
  const answers = turn.members
    ...
    .map((member) => {
      ...
      if (member.kind === "work_log")
        return member.turns.map(lastTurnText).find((text) => text.trim()) ?? "";
      return "";
    })
    .filter((text) => text.trim());

  const raw =
    answers[answers.length - 1] ?? (head.kind === "user" ? head.content : "");
  return { title, body: stripLabelStem(title, raw) };
}

export const SessionNavigationPanel = memo(function SessionNavigationPanel({
  scrollRootRef,
}: Props) {
  const { t } = useLocale();
  const {
  ...
  useEffect(() => {
    ...
    const measureOffsets = () => {
      ...
      for (const turn of turns) {
        ...
        next.push({
          id: turn.id,
          top: element.getBoundingClientRect().top - rootTop + root.scrollTop,
        });
      }
      offsets = next;
    };
  ...
  const preview = hoveredTurn
    ? turnPreview(hoveredTurn, t("sessionNavEmpty"))
    : null;
  ...
  return (
    ...
    >
      <div
        className="session-nav-float-rail"
      ...
      >
        {preview ? (
          <div className="session-nav-preview" role="tooltip">
            ...
            {preview.body ? (
              <div className="session-nav-preview-body">{preview.body}</div>
            ) : null}
          </div>
        ) : null}
        ...
        <ol className="session-nav-float-list">
          {turns.map((turn, index) => {
            const distance =
              hoverIndex === null ? null : Math.abs(index - hoverIndex);
            const width =
              distance === null
                ? RESTING_WIDTH
            ...
            return (
              <li key={turn.id} className="session-nav-float-item">
                <button
                  ...
                  aria-label={t("sessionNavJumpTo", {
                    index: index + 1,
                    label: turn.members[0].label || t("sessionNavEmpty"),
                  })}
