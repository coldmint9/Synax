...
export function SessionQuickToolbar() {
  ...
  const runningCount = sessions.filter(
    (s) => s.status === "running" || s.status === "waiting_permission",
  ).length;
    ...
    <>
      <div className="session-quick-toolbar">
        <div className="flex items-center gap-1.5 text-[11px] text-muted-foreground">
          <Radio
            size={12}
            className={runningCount > 0 ? "text-[var(--color-run)]" : ""}
          />
            ...
            {sessions
              ...
              .map((session) => (
                <button
                  ...
                  type="button"
                  onClick={() =>
                    selectedSessionId === session.id && panelOpen
                      ? closePanel()
        ...
        {sessions.length > 0 && (
          <button
            ...
            onClick={() =>
              panelOpen
                ? closePanel()
                : sessions[0] && openPanel(sessions[0].id)
            }
