      ...
      <div className="flex items-center gap-3 text-[9px] text-muted-foreground">
        {stats.activeSubAgentCount > 0 && (
          <span className="flex items-center gap-1">
            <Users size={9} />
            {stats.activeSubAgentCount} active
          </span>
        )}
      </div>
    </div>
...
export function SessionModeSummary({ session }: { session: AgentSession }) {
  ...
  const status = goal
    ? (GOAL_STATUS_LABELS[goal.status]?.[zh ? 0 : 1] ?? goal.status)
    : plan
      ? plan.status === "approved"
        ...
        : plan.status === "saved"
          ? zh
            ? "已保存，等待执行指令"
            : "Saved, awaiting execution instruction"
          : zh
            ? "草稿"
            : "Draft"
      : null;
  return (
    <details
      ...
      aria-label={zh ? "会话目标和角色" : "Session goal and specialist"}
    >
      <summary>
        <ChevronRight
          size={11}
          aria-hidden
          className="agent-disclosure-arrow"
        />
        <span className="agent-summary-icon" aria-hidden="true">
          <Target size={15} />
        </span>
        {status && (
          <span role="status" className="agent-summary-status">
            {status}
          </span>
        )}
        <span className="agent-summary-title" title={title ?? undefined}>
          {title}
        </span>
      </summary>
      ...
      {goal?.reason && (
        <p className="pb-1 text-[11px] leading-relaxed text-warning">
          {goal.reason}
        </p>
      )}
    </details>
        ...
        {files.map((f) => {
          ...
          return (
            ...
            >
              <Icon
                size={10}
                className={`shrink-0 ${CHANGE_COLOR[f.changeType]}`}
              />
...
export const SessionWorkspace = memo(function SessionWorkspace() {
  const session = useAgentSessionStore((s) =>
    s.sessions.find((item) => item.id === s.selectedSessionId),
  );
  const {
    events,
    sessionStats,
  ...
  const fileChanges = useMemo<FileChange[]>(() => {
    ...
    for (const item of events) {
      ...
      if (match)
        paths.set(match[1], { path: match[1], changeType: "modified" });
    }
    return [...paths.values()];
  }, [events]);
