...
describe("SessionTreeItem", () => {
  ...
  it("keeps completed sessions quiet by default and shows a dot after a new update", () => {
    ...
    expect(useAgentSessionStore.getState().readSessionMarkers[session.id]).toBe(
      session.updatedAt,
    );
    ...
    rerender(
      <SessionTreeItem
        node={makeNode({ ...session, updatedAt: "2026-01-03T00:00:00Z" })}
        isSelected={false}
        onSelect={onSelect}
        onToggleExpand={noop}
      />,
  ...
  it("keeps the running indicator after selection", () => {
    ...
    const { container } = render(
      <SessionTreeItem
        node={makeNode(session)}
        isSelected={false}
        onSelect={(id) => useAgentSessionStore.getState().markSessionRead(id)}
        onToggleExpand={noop}
      />,
    );
    fireEvent.click(container.querySelector(".session-list-select")!);
  ...
  it("shows the latest conversation message and excludes tool output and injected prompts", () => {
    ...
    const { container } = render(
      <SessionTreeItem
        node={makeNode(makeSession({ resultSummary: "older summary" }))}
        isSelected
        onSelect={noop}
        onToggleExpand={noop}
      />,
  ...
  it("uses the summary or task content when no messages have been loaded", () => {
    const { container, rerender } = render(
      <SessionTreeItem
        node={makeNode(makeSession({ resultSummary: "已完成布局优化" }))}
        isSelected={false}
        onSelect={noop}
        onToggleExpand={noop}
      />,
