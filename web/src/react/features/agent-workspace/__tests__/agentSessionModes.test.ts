...
import {
  RuntimeStreamProjector,
  type RuntimeStreamRecord,
} from "../../../../lib/api/runtimeStream";
...
const session: AgentSession = {
  ...
  model: null,
  sessionMetadata: { mode: "chat", permissionTier: "boundary" },
};
const interaction: AgentInteraction = {
  ...
  revision: 1,
  status: "pending",
  request: { title: "Question", questions: [] },
  response: null,
  createdAt: "",
  resolvedAt: null,
};
...
function mockRunTransport(state: RuntimeStreamRecord["state"]) {
  ...
  vi.mocked(ensureSessionLiveSubscription).mockImplementation(
    (_id, handler) => {
      deliver = handler;
    },
  );
...
describe("session mode boundaries", () => {
  it.each([
    ...
    "waiting_input",
    "waiting_permission",
  ] as const)("does not switch while %s", (status) => {
  ...
  it("only permits an idle native primary without pending input or permissions", () => {
    ...
    expect(
      canSwitchSessionMode(session, { hasPendingInteractions: true }),
    ).toBe(false);
    expect(canSwitchSessionMode(session, { hasPendingPermissions: true })).toBe(
      false,
    );
    ...
    expect(
      canSwitchSessionMode({
        ...session,
        pendingResumeToken: "interaction:i1",
      }),
    ).toBe(false);
    expect(
      canSwitchSessionMode({ ...session, parentSessionId: "parent" }),
    ).toBe(false);
    expect(
      canSwitchSessionMode({
        ...session,
        sessionMetadata: { mode: "plan_node" },
      }),
    ).toBe(false);
    expect(
      canSwitchSessionMode({ ...session, model: "codex-acp/default" }),
    ).toBe(false);
    expect(
      canSwitchSessionMode({
        ...session,
        sessionMetadata: { acp: { engineModel: "pi-acp/default" } },
      }),
    ).toBe(false);
  ...
  it("marks waiting input unread and locks free text rather than enqueueing default answers", () => {
    ...
    expect(
      isSessionComposerLocked(session, { hasPendingInteractions: true }),
    ).toBe(true);
  });
  ...
  it.each(["chat", "plan", "goal"] as const)(
    "creates %s mode in metadata without changing permissions",
    async (mode) => {
      ...
      expect(agentRuntimeApi.createSession).toHaveBeenCalledWith(
        expect.objectContaining({
          permissionTier: "boundary",
          sessionMetadata: expect.objectContaining({ mode }),
        }),
      );
    },
  );
  ...
  it("uses the server mode metadata and preserves it if a later switch fails", async () => {
    vi.spyOn(agentRuntimeApi, "updateSessionMode")
      ...
      .mockRejectedValueOnce(new Error("Run started"));
    await useAgentSessionStore.getState().updateSessionMode("s1", "goal");
    expect(
      useAgentSessionStore.getState().sessions[0].sessionMetadata,
    ).toMatchObject({
      mode: "goal",
      permissionTier: "boundary",
    ...
    await expect(
      useAgentSessionStore.getState().updateSessionMode("s1", "plan"),
    ).rejects.toThrow("Run started");
    expect(
      useAgentSessionStore.getState().sessions[0].sessionMetadata?.mode,
    ).toBe("goal");
  });
  it.each([
    null,
    { sessionId: "s1", items: [interaction], loading: false, error: null },
    { sessionId: "s1", items: [], loading: false, error: "Offline" },
  ])(
    "fails closed before mode PATCH when pending requests are unknown or unresolved",
    async (interactionState) => {
      useAgentSessionStore.setState({ interactionState });
      vi.spyOn(agentRuntimeApi, "updateSessionMode");
      await expect(
        useAgentSessionStore.getState().updateSessionMode("s1", "plan"),
      ).rejects.toThrow("idle native session");
  ...
  it("does not turn a durable waiting event followed by stream done into completion", async () => {
    ...
    mockRunTransport({
      ...
      activeRunId: "r1",
      pendingResumeToken: "question",
      blockedReason: "Answer needed",
      updatedAt: "",
    });
    await useAgentSessionStore
      .getState()
      .sendSessionMessage("s1", { message: "Task" });
...
describe("durable interaction refresh races", () => {
  ...
  it("ignores an old GET after a reply has committed", async () => {
    ...
    await refresh;
    expect(
      useAgentSessionStore.getState().interactionState?.items[0].status,
    ).toBe("answered");
  });
