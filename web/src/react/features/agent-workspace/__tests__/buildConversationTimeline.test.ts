...
function makeMessage(
  partial: Partial<AgentRuntimeMessage>,
): AgentRuntimeMessage {
  return {
    id: "msg-user",
    sessionId: SESSION_ID,
...
describe("buildUserMessageEntries", () => {
  ...
  it.each(["userPrompt", "goalContent"])(
    "includes %s as the first user bubble when initial run used session_prompt",
    (field) => {
      const session: AgentSession = {
        ...
        sessionMetadata: {
          [field]: "你好",
          source: field === "userPrompt" ? "agent-dock" : "goal-dock",
        },
      };
...
describe("inline interactions", () => {
  const interaction: AgentInteraction = {
    ...
    request: {
      title: "Which scope?",
      questions: [{ id: "scope", type: "text", label: "Scope" }],
    },
  ...
  it.each([true, false])(
    "keeps a question and its answer at the original step with foldWorkRuns=%s",
    (foldWorkRuns) => {
      const entries = buildConversationTimeline(
        ...
        [
          makeMessage({
            ...
            role: "assistant",
            stepId: STEP_ID,
            content: "Let us choose the scope.",
          }),
          makeMessage({
            id: "next",
            role: "assistant",
            stepId: "step-2",
            content: "Proceeding with frontend.",
          }),
        ],
        [],
        [],
      ...
      expect(entries[index]).toMatchObject({
        interaction: { response: { answers: { scope: "Frontend" } } },
      });
      expect(
        entries
          .slice(index + 1)
          .some(
            (entry) => entry.kind === "agent" && entry.turn.stepId === "step-2",
          ),
      ).toBe(true);
      expect(
        entries.filter((entry) => entry.kind === "interaction"),
      ).toHaveLength(1);
    },
  );
  it("renders a durable pending question even before the step snapshot arrives", () => {
    const entries = buildConversationTimeline([], [], [], [], [], {
      interactions: [{ ...interaction, status: "pending", response: null }],
    });
    expect(entries).toHaveLength(1);
