...
import { isSessionUnread } from "../state/agentSessionStore";
...
describe("isSessionUnread", () => {
  it("does not mark unseen completed sessions as unread", () => {
    expect(isSessionUnread(makeSession({ status: "completed" }), {})).toBe(
      false,
    );
  });
  ...
  it("marks unseen active sessions as unread", () => {
    ...
    expect(
      isSessionUnread(makeSession({ status: "waiting_permission" }), {}),
    ).toBe(true);
  });
  ...
  it("respects persisted read markers", () => {
    ...
    expect(isSessionUnread(session, { "sess-1": "2026-01-02T00:00:00Z" })).toBe(
      false,
    );
  });
