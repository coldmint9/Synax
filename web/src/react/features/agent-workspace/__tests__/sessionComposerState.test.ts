...
describe("sessionComposerState", () => {
  it("locks while submitting", () => {
    expect(isSessionComposerLocked(makeSession(), { submitting: true })).toBe(
      true,
    );
  });
  ...
  it("unlocks completed sessions", () => {
    expect(isSessionComposerLocked(makeSession({ status: "completed" }))).toBe(
      false,
    );
  });
  ...
  it("unlocks running sessions without activeRunId", () => {
    expect(
      isSessionComposerLocked(
        makeSession({ status: "running", activeRunId: null }),
      ),
    ).toBe(false);
  });
  ...
  it("locks waiting_permission only when pending approvals exist", () => {
    ...
    expect(
      isSessionComposerLocked(session, { hasPendingPermissions: false }),
    ).toBe(false);
    expect(
      isSessionComposerLocked(session, { hasPendingPermissions: true }),
    ).toBe(true);
  });
  ...
  it("allows enqueue while running or waiting permission", () => {
    expect(
      canEnqueueSessionInput(
        makeSession({ status: "running", activeRunId: "run-1" }),
      ),
    ).toBe(true);
    expect(
      canEnqueueSessionInput(
        makeSession({ status: "waiting_permission", activeRunId: "run-1" }),
      ),
    ).toBe(true);
    expect(canEnqueueSessionInput(makeSession({ status: "completed" }))).toBe(
      false,
    );
  ...
  it("patches session fields locally", () => {
    ...
    expect(
      patchAgentSession(session, { status: "completed", activeRunId: null }),
    ).toMatchObject({
      status: "completed",
      activeRunId: null,
    });
  ...
  it("detects pending permissions only for the selected session", () => {
    ...
    expect(sessionHasPendingPermissions("sess-1", "sess-1", permissions)).toBe(
      true,
    );
    expect(sessionHasPendingPermissions("sess-1", "sess-2", permissions)).toBe(
      false,
    );
    expect(
      sessionHasPendingPermissions("sess-1", "sess-1", [
