...
describe("sessionLastVisit", () => {
  ...
  it("persists new draft visit", () => {
    ...
    expect(sessionLastVisitPath(PROJECT_ID, { kind: "new" })).toBe(
      "/projects/p-test/sessions/new",
    );
  });

  it("persists session visit", () => {
    saveSessionLastVisit(PROJECT_ID, { kind: "session", sessionId: "ars_1" });
    expect(loadSessionLastVisit(PROJECT_ID)).toEqual({
      kind: "session",
      sessionId: "ars_1",
    });
    expect(resolveSessionsEntryPath(PROJECT_ID)).toBe(
      "/projects/p-test/sessions?session=ars_1",
    );
  });

  it("falls back to bare list when no visit saved", () => {
    expect(resolveSessionsEntryPath(PROJECT_ID)).toBe(
      "/projects/p-test/sessions",
    );
  });
});
