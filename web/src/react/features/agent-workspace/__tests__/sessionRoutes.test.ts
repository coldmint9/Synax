...
describe("sessionRoutes", () => {
  it("builds session paths", () => {
    ...
    expect(sessionPath("p1", "ars_1")).toBe(
      "/projects/p1/sessions?session=ars_1",
    );
    expect(workflowSessionsPath("p1")).toBe("/projects/p1/sessions/workflows");
    expect(workflowSessionPath("p1", "ars_wf")).toBe(
      "/projects/p1/sessions/workflows?session=ars_wf",
    );
  });
  ...
  it("detects bare sessions list", () => {
    ...
    expect(isBareSessionsPath("/projects/p1/sessions/workflows", "p1")).toBe(
      false,
    );
    expect(isBareSessionsPath("/projects/p1/sessions?session=x", "p1")).toBe(
      false,
    );
  });
});
