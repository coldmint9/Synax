...
describe("workspace layout constraints", () => {
  ...
  it("keeps the project header in place both at rest and when hovered", () => {
    const header =
      ":is(.work-page, .work-details-dialog) .ws-project-card > .ws-card-head";
  ...
  it("uses matching row padding and unclipped icon slots instead of offsets", () => {
    const repository =
      ":is(.work-page, .work-details-dialog) .ws-project-repository";
    ...
    expect(
      declaration(`${branch} > svg:first-child`, "inset-inline-start"),
    ).toBeUndefined();
