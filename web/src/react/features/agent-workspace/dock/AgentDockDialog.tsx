...
export function AgentDockDialog({
  status,
  sessionTitle,
  toolCalls,
  thinking,
    ...
    <div className="agent-dock-dialog relative w-full rounded-2xl border border-white/55 bg-white/48 p-3.5 pb-3 shadow-lg backdrop-blur-xl dark:border-border/50 dark:bg-card/90">
      {onOpenSession && (
        <button
          ...
          aria-label={t("agentOpenFullSession")}
      ...
      {error && <p className="mb-2 text-[11px] text-destructive/90">{error}</p>}

      <div className="agent-dock-dialog-scroll max-h-[14rem] space-y-2 overflow-y-auto">
        {thinking.trim() && (
          <ThinkingBlock
            content={thinking}
            isStreaming={isRunning && !recent.length}
          />
        )}
      ...
      {onReplyPermission && listPendingPermissions(permissions).length > 0 && (
        <div className="agent-dock-dialog-permissions mt-2 border-t border-border/25 pt-2">
          <AgentQuickApproval
