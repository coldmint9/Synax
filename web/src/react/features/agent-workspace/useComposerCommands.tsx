...
function findQuery(value: string, cursor: number): Query | null {
  ...
  if (detail)
    return {
      ...
      command: detail[1] as TurnReference["kind"],
      search: detail[2],
    };
  const root = /(?:^|\s)\/([a-z]*)$/i.exec(before);
  return root
    ? { start: cursor - root[1].length - 1, end: cursor, search: root[1] }
    : null;
}
...
export function useComposerCommands({
  ...
  setReferences: (value: TurnReference[]) => void;
  ...
  const unavailable = (id: CommandId) => {
    if ((id === "skill" || id === "mcp") && !native)
      return zh ? "此后端使用原生配置" : "Managed by this backend";
    if (["plan", "goal"].includes(id) && !modeEnabled)
      return zh
        ? "仅支持 Synax，且须会话空闲、无待处理请求"
        : "Requires an idle Synax session without pending requests";
    return undefined;
  };
  const rows = query?.command
    ? options.map((ref) => ({
        ...
        detail: ref.label !== ref.id ? ref.id : "",
        command: ref.kind as CommandId,
        ref,
        disabled: references.some(
    ...
    : commands
        ...
        .map((cmd) => ({
          id: cmd.id,
          title: `/${cmd.id}`,
          detail: zh ? cmd.zh : cmd.en,
          command: cmd.id,
          ref: undefined as TurnReference | undefined,
          disabled: unavailable(cmd.id),
        }));
  ...
  const replaceQuery = (replacement: string, keepOpen = false) => {
    if (!query) return;
    const value =
      content.slice(0, query.start) + replacement + content.slice(query.end);
  ...
  const select = async (index: number) => {
    ...
    if (row.ref) {
      if (references.length >= 20) {
        setError(zh ? "最多添加 20 个引用" : "Up to 20 references");
        return;
      }
      setReferences([...references, row.ref]);
      replaceQuery("");
      return;
    }
  ...
  const onKeyDown = (event: KeyboardEvent<HTMLTextAreaElement>) => {
    ...
    if (event.key === "Enter" || event.key === "Tab") {
      event.preventDefault();
      if (!loading) void select(Math.min(active, Math.max(rows.length - 1, 0)));
      return true;
    }
    return false;
  };
  ...
  const header = references.length > 0 && (
    <div
      className="session-composer-reference-tags"
      aria-label={zh ? "本条消息引用" : "References for this message"}
    >
      {references.map((ref) => {
        const Icon = commands.find((cmd) => cmd.id === ref.kind)!.Icon;
        return (
          <span
            key={`${ref.kind}:${ref.id}`}
            className="session-composer-reference-tag"
            data-kind={ref.kind}
            title={ref.id}
          >
            <Icon size={12} />
            ...
            <button
              type="button"
              disabled={disabled}
              aria-label={`${zh ? "移除" : "Remove"} ${ref.label ?? ref.id}`}
              onClick={() =>
                setReferences(references.filter((item) => item !== ref))
              }
            >
              <X size={12} />
            </button>
  ...
  const menu =
    ...
    createPortal(
      <div
        ref={menuRef}
        className="session-composer-command-menu"
        style={menuPosition}
        onMouseDown={(event) => event.preventDefault()}
      >
        <div className="session-composer-command-heading">
          {query.command
            ? `/${query.command} · ${zh ? "输入名称搜索" : "Search by name"}`
            : zh
              ? "选择命令"
              : "Choose a command"}
        </div>
        <div
          id={listId}
          role="listbox"
          aria-label={zh ? "斜杠命令" : "Slash commands"}
        >
          {rows.map((row, index) => {
            const Icon = commands.find((cmd) => cmd.id === row.command)!.Icon;
            return (
              <div
                key={row.id}
                id={`${listId}-${index}`}
                ...
                className="session-composer-command-option"
              ...
              >
                ...
                <span>
                  ...
                  <small>{row.disabled || row.detail}</small>
                </span>
              </div>
            );
          })}
        ...
        {!loading && rows.length === 0 && (
          <p role={error ? "alert" : "status"}>
            {error || (zh ? "没有匹配项" : "No matches")}
          </p>
        )}
        {error && rows.length > 0 && <p role="alert">{error}</p>}
        <div className="session-composer-command-help">
          <span>↑ ↓ {zh ? "选择" : "navigate"}</span>
          <span>↵ {zh ? "确认" : "select"}</span>
  ...
  return {
    overlayOpen: contextOpen || Boolean(query),
    inputRef,
    onInput: updateQuery,
    onKeyDown,
    header,
    ...
    activeId: rows.length
      ? `${listId}-${Math.min(active, rows.length - 1)}`
      : undefined,
  };
}
