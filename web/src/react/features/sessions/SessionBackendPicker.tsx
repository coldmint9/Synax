...
export function SessionBackendPicker({ value, options, disabled, onChange }: {
  value: BackendId
  ...
  return <Popover isOpen={!disabled && open} onOpenChange={next => setOpen(!disabled && next)}>
    <Popover.Trigger<'button'> render={props => <button {...props} type="button" />}
      disabled={disabled} aria-label={zh ? '执行后端' : 'Execution backend'}
      title={disabled ? (zh ? '后端在会话创建时固定；切换后端请新建会话。' : 'Backend is fixed for this session. Start a new session to switch.') : undefined}
      className="goal-dock-composer-chip agent-mode-trigger">
      <span>{options.find(option => option.id === value)?.label ?? value}</span><ChevronDown size={10} aria-hidden />
    </Popover.Trigger>
    <Popover.Content placement="top end" offset={8} className="agent-mode-popover">
      <ListBox aria-label={zh ? '执行后端' : 'Execution backend'} selectionMode="single" disallowEmptySelection
        selectedKeys={new Set([value])} onSelectionChange={keys => {
          ...
          if (option) { setOpen(false); onChange(option.id) }
        }}>
