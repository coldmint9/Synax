...
export const SessionProfilePanel = memo(function SessionProfilePanel({ sessionId }: { sessionId: string | null }) {
  ...
  const { loading, sessionStats, sessionCapabilities, steps, runs, session } = useAgentSessionStore(useShallow(s => ({
    loading: s.detailLoading,
    sessionStats: s.selectedSessionId === sessionId ? s.sessionStats : null,
    sessionCapabilities: s.selectedSessionId === sessionId ? s.sessionCapabilities : null,
    steps: s.steps,
    runs: s.runs,
  ...
  return (
    <div className="work-runtime-details">
      <WorkspaceSection key={sessionId} icon={<SlidersHorizontal size={13} />} title={locale === 'zh' ? '运行详情' : 'Runtime details'} defaultOpen={false}>
        {loading && !sessionStats ? <Skeleton className="m-3 h-12 rounded-lg" /> : <>
          ...
          {!sessionStats && !sessionCapabilities && <p className="ws-empty">{locale === 'zh' ? '暂无运行数据' : 'No runtime data yet'}</p>}
        </>}
      </WorkspaceSection>
    </div>
  )
