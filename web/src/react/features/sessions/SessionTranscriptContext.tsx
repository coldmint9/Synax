...
export function TranscriptSessionProvider({
  ...
  return (
    <TranscriptSessionContext.Provider value={{ sessionId: sessionId ?? null, workspacePath }}>
      {children}
...
export function useTranscriptSession(): TranscriptSessionScope {
  ...
  if (!scoped) {
    return { sessionId: polled.sessionId, workspacePath: polled.environment?.workspacePath ?? null }
  }

  const inherited = polled.sessionId === scoped.sessionId ? polled.environment?.workspacePath ?? null : null
  return { sessionId: scoped.sessionId, workspacePath: scoped.workspacePath ?? inherited }
}
