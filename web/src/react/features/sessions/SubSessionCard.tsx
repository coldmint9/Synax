      ...
      <div className="bui-task-header">
        ...
        >
          <span className="bui-task-copy">
            ...
            <span className="bui-task-title" title={title}>{title}</span>
          </span>
          <ActivityStatus status={session.status} />
          {expanded ? <ChevronDown size={12} className="bui-chevron" aria-hidden="true" /> : <ChevronRight size={12} className="bui-chevron" aria-hidden="true" />}
        </button>
        {onExpand && (
          <button type="button" onClick={() => onExpand(session.id)} className="bui-task-open">
            {t('subSessionDetails')}<ArrowUpRight size={11} aria-hidden="true" />
          </button>
        )}
      </div>
      {expanded && (
