...
export function ToolCallSummaryLine({ call }: Props) {
  ...
  const presentation = useMemo(() => toolCallPresentation(call), [call.toolId, call.category, call.inputSummary])
  ...
  const heading = (
    <>
      ...
      <span className="bui-tool-meta">
        ...
        <ActivityStatus status={call.status} compact={call.status === 'completed'} />
        {hasDetails && (expanded
          ? <ChevronDown size={11} className="bui-chevron" aria-hidden="true" />
          : <ChevronRight size={11} className="bui-chevron" aria-hidden="true" />)}
      </span>
    </>
  )
    ...
    <div className="bui-tool" data-tool-status={call.status}>
      ...
      {expanded && hasDetails && (
        <div id={detailsId} className="bui-tool-details">
          ...
          {call.inputSummary && (
            <div>
              <div className="bui-tool-detail-label">{t('activityToolInput')}</div>
              <pre>{call.inputSummary}</pre>
            </div>
          )}
          {call.outputSummary && (
            <div>
              <div className="bui-tool-detail-label">{t('activityToolOutput')}</div>
