...
import type { SessionEnvironment, SessionEnvironmentFile } from '../../../../lib/api/agentRuntime'

const getSessionEnvironment = vi.fn()
const openWorkspaceDiff = vi.fn()

vi.mock('../../../../lib/api/agentRuntime', () => ({
  agentRuntimeApi: { getSessionEnvironment: (...args: unknown[]) => getSessionEnvironment(...args) },
}))

vi.mock('../sessionWorkspaceStore', () => ({
  openWorkspaceDiff: (...args: unknown[]) => openWorkspaceDiff(...args),
}))

const { SessionFileChangeIsland } = await import('../SessionFileChangeIsland')

function file(overrides: Partial<SessionEnvironmentFile> = {}): SessionEnvironmentFile {
  return {
    path: 'src/app.ts',
    status: 'modified',
    additions: 6,
...
function environment(agentChangedFiles: SessionEnvironmentFile[]): SessionEnvironment {
  return {
    sessionId: 'sess-1',
    projectId: 'p1',
...
async function renderIsland() {
  const utils = render(<SessionFileChangeIsland sessionId="sess-1" isRunning={false} />)
...
describe('SessionFileChangeIsland', () => {
  ...
  it('summarises the session files with added and removed line counts', async () => {
    getSessionEnvironment.mockResolvedValue(environment([
      ...
      file({ path: 'src/new.ts', status: 'added', additions: 12, deletions: 0 }),
    ]))
    await renderIsland()
  ...
  it('lists the changed files and opens a diff tab when one is picked', async () => {
    getSessionEnvironment.mockResolvedValue(environment([file({ path: 'src/app.ts' })]))
    await renderIsland()
