...
vi.mock('../ToolCallRoundPanel', () => ({
  ToolCallRoundPanel: ({ toolBlocks, isStreaming }: { toolBlocks: TurnContentBlock[]; isStreaming?: boolean }) => (
    <div data-live-tools={isStreaming || undefined}>{toolBlocks.map(block => block.type === 'tool_call' ? block.call.inputSummary : '').join('')}</div>
  ),
}))
...
describe('SessionLiveTurn', () => {
  ...
  it('keeps the current running step in the live layer', () => {
    const { container } = render(<SessionLiveTurn {...props} steps={[{ ...persisted, status: 'running' }]}
      streamingStepId="step-1" streamingCompletedSteps={[]}
      streamingLive={{ ...EMPTY_STREAMING_BUFFERS, blocks }} />)
    expect(screen.getAllByText('Locating BUI styles elsewhere')).toHaveLength(1)
    expect(screen.getAllByText('bui-tool|bui-activity|StreamingTextBlock')).toHaveLength(1)
    expect(container.querySelectorAll('[data-live-tools="true"]')).toHaveLength(1)
  })
})
