...
type ObserverCallback = (records: Array<{ isIntersecting: boolean; target: Element }>) => void
...
function userEntry(id: string, content: string): ConversationTimelineEntry {
  return { id, kind: 'user', createdAt: '2026-01-01T00:00:00Z', label: id, content }
}
...
describe('TimelineLazyEntry', () => {
  ...
  it('reserves height and defers the body until it reaches the viewport', async () => {
    ...
    await act(async () => { FakeIntersectionObserver.instances[0]?.emit(true) })
