  ...
  it('keeps the latest activity waiting after tools finish, and removes the dots when collapsed or stopped', () => {
    const turn: InterleavedTurn = {
      ...
      blocks: [{ type: 'tool_call', call: {
        id: 'call-1', toolId: 'bash', inputSummary: 'pwd', outputSummary: '/workspace',
        status: 'completed', duration: '10ms',
      } }],
    }
    const { container, rerender } = render(<TurnBody turn={turn} entryId="entry-1" isWorking />)
    const heading = container.querySelector('.bui-thinking-trigger') as HTMLButtonElement
    ...
    expect(container.querySelector('[data-activity-body] [data-thinking-dot]')).toBeNull()
