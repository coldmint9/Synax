...
describe('resolveActiveEntryIndex', () => {
  ...
  it('selects the last turn when scrolled to the very bottom', () => {
    expect(resolveActiveEntryIndex(OFFSETS, MAX_SCROLL, VIEWPORT, MAX_SCROLL)).toBe(3)
    expect(resolveActiveEntryIndex(OFFSETS, MAX_SCROLL - 1, VIEWPORT, MAX_SCROLL)).toBe(3)
  })

  it('selects the turn under the probe line in between', () => {
    // probe = 1000 + 350 = 1350 -> inside the second turn
    expect(resolveActiveEntryIndex(OFFSETS, 1000, VIEWPORT, MAX_SCROLL)).toBe(1)
    // probe = 2000 + 350 = 2350 -> inside the third turn
    expect(resolveActiveEntryIndex(OFFSETS, 2000, VIEWPORT, MAX_SCROLL)).toBe(2)
  })
