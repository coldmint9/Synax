...
function userEntry(id: string, content: string, injected = false): ConversationTimelineEntry {
  return {
    id,
    kind: 'user',
...
function agentTurn(id: string, blocks: InterleavedTurn['blocks']): InterleavedTurn {
  return { stepId: id, index: 1, status: 'completed', duration: null, blocks }
}

function agentEntry(id: string, blocks: InterleavedTurn['blocks'], label = ''): ConversationTimelineEntry {
  return {
    id,
    kind: 'agent',
    createdAt: `2026-09-10T10:00:${id.padStart(2, '0')}.000Z`,
...
function workLogEntry(id: string, turns: InterleavedTurn[]): ConversationTimelineEntry {
  return {
    id,
    kind: 'work_log',
    createdAt: `2026-09-10T10:00:${id.padStart(2, '0')}.000Z`,
    ...
    turns,
    stats: { stepCount: turns.length, toolCallCount: 0, thinkingChars: 0, elapsedMs: 0 },
  }
}
...
describe('groupTimelineIntoTurns', () => {
  it('folds every agent step of a turn under the prompt that opened it', () => {
    const turns = groupTimelineIntoTurns([
      ...
      agentEntry('3', [{ type: 'tool_call', call: { id: 't1', toolId: 'bash', inputSummary: 'ls', outputSummary: '', status: 'completed', category: 'shell', duration: '3ms', mutability: 'read' } }]),
  ...
  it('starts a new tick only on the next user message', () => {
    const turns = groupTimelineIntoTurns([
      ...
      workLogEntry('3', [agentTurn('s1', [{ type: 'thinking', content: '嗯' }]), agentTurn('s2', [{ type: 'thinking', content: '嗯' }])]),
      userEntry('4', '再改一处'),
      agentEntry('5', [{ type: 'text', content: '完成' }]),
    ])

    expect(turns.map(turn => turn.id)).toEqual(['1', '4'])
    expect(turns[0].members.map(member => member.id)).toEqual(['1', '2', '3'])
...
describe('turnPreview', () => {
  it('shows the prompt above the last answer of the turn', () => {
    const [turn] = groupTimelineIntoTurns([
      ...
      agentEntry('3', [{ type: 'text', content: '收到，Ekko。16 个文件已完成。' }], '收到，Ekko。…'),
    ])
  ...
  it('never echoes injected scaffolding and strips the duplicated label stem', () => {
    ...
    const [agentHeaded] = groupTimelineIntoTurns([
      agentEntry('1', [{ type: 'text', content: '收到，Ekko。已修复。' }], '收到，Ekko。…'),
    ])
    expect(turnPreview(agentHeaded)).toEqual({ title: '收到，Ekko。…', body: '已修复。' })
  })
})
