...
export function projectSessionBadge({
  ...
  for (const session of sessions) {
    ...
    if (Number.isFinite(updatedAt) && age >= 0 && age <= NEW_COMPLETION_WINDOW_MS) {
      unreadCompleted += 1
    }
  }
...
export function useProjectSessionBadges(
  ...
  }, [clearScheduledRefresh, refresh, scheduleEventRefresh, stableProjectIds.length])

  useEffect(() => {
    let nextExpiry = Number.POSITIVE_INFINITY
    for (const sessions of Object.values(sessionsByProject)) {
      for (const session of sessions) {
        if (!COMPLETED_SESSION_STATUSES.has(session.status) || readMarkers[session.id]) continue
        const updatedAt = Date.parse(session.updatedAt)
