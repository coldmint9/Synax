import { FileEdit, FileSearch, FolderOpen, GitBranch, Search, Terminal, Wrench } from 'lucide-react'
...
const CATEGORY_ICONS: Record<string, typeof Wrench> = {
  external_execution: Terminal, shell: Terminal, write: FileEdit, read: FileSearch,
  search: Search, context: Search, task: GitBranch,
}
...
export function toolCallPresentation(call: ToolCallView) {
  ...
  try {
    ...
    if (input && typeof input === 'object' && !Array.isArray(input)) {
      ...
      for (const key of ['command', 'query', 'pattern', 'url', 'path', 'file_path', 'name']) {
    ...
    const match = /(?:^|[,{])\s*"(?:command|query|pattern|url|path|file_path|name)"\s*:\s*("(?:[^"\\]|\\.)*")/.exec(call.inputSummary)
    if (match) {
      try { target = JSON.parse(match[1]) as string } catch { /* Fall back to the unparsed summary. */ }
    }
  }
  if (!target) {
  ...
  return { label: tool?.label, icon: tool?.icon ?? CATEGORY_ICONS[call.category] ?? Wrench, target }
}

export function aggregateToolStatus(calls: ToolCallView[]): string {
  for (const status of ['failed', 'running', 'denied', 'cancelled', 'pending']) {
