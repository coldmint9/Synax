...
vi.mock("../TerminalViewport", async () => {
  ...
  return {
    TerminalViewport: ({ session, visible }: any) => {
      useEffect(() => {
        mocks.mounted(session.id);
        return () => mocks.unmounted(session.id);
      }, [session.id]);
      return <div hidden={!visible}>terminal:{session.id}</div>;
    },
  };
});
...
const item = (id: string, cwd = "/project"): TerminalSession => ({
  id,
  projectId: "p",
  rootId: "p",
  ownerSessionId: null,
...
it("keeps terminal views mounted across tab switches and drawer hiding", async () => {
  useTerminalStore.getState().accept(item("one", "/workspaces/alpha"));
  useTerminalStore.getState().accept(item("two", "/workspaces/beta"));
...
it("creates a terminal automatically when the drawer opens without tabs", async () => {
  ...
  await waitFor(() =>
    expect(terminalApi.create).toHaveBeenCalledWith(
      "p",
      expect.objectContaining({ rootId: "p", sessionId: undefined }),
    ),
  );
  expect(await screen.findByText("terminal:auto")).toBeVisible();
});
it("restores a manually created terminal without mixing in services", async () => {
  vi.mocked(terminalApi.list).mockResolvedValue({ items: [item("existing")] });
  useTerminalStore.setState({ open: false, tabs: [], activeId: null });
  render(<TerminalDrawer projectId="p" sessionId={null} />);
  act(() => useTerminalStore.getState().show());
  expect(await screen.findByText("terminal:existing")).toBeVisible();
  expect(terminalApi.create).not.toHaveBeenCalled();
});
it("keeps legacy services untouched until the explicit restart confirmation is submitted", async () => {
  ...
  await userEvent.click(
    screen.getByRole("button", { name: "确认停止旧服务并重启到终端" }),
  );
  await waitFor(() =>
    expect(terminalApi.restartLegacy).toHaveBeenCalledWith(
      "s",
      "old",
      expect.objectContaining({ command: "npm run dev", cwd: "/project" }),
    ),
  );
...
it("disables deleting a live terminal and enables it after the terminal exits", async () => {
  ...
  act(() =>
    useTerminalStore.getState().update({ ...item("one"), state: "closed" }),
  );
