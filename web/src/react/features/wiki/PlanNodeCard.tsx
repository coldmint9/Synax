...
function NodeActions({ index, isLast, node, onMoveUp, onMoveDown, onEdit, onDelete }: Pick<Props, 'index' | 'isLast' | 'node' | 'onMoveUp' | 'onMoveDown' | 'onEdit' | 'onDelete'>) {
  return (
    <div className="flex items-center gap-0.5 opacity-0 group-hover:opacity-100 transition-opacity">
      {onMoveUp && index > 0 && (
        <Button variant="light" size="sm" isIconOnly onPress={() => onMoveUp(node.id)} className="min-w-6 h-6"><ChevronUp size={12} /></Button>
      )}
      {onMoveDown && !isLast && (
        <Button variant="light" size="sm" isIconOnly onPress={() => onMoveDown(node.id)} className="min-w-6 h-6"><ChevronDown size={12} /></Button>
      )}
      {onEdit && (
        <Button variant="light" size="sm" isIconOnly onPress={() => onEdit(node)} className="min-w-6 h-6"><Pencil size={12} /></Button>
      )}
      {onDelete && (
        <Button variant="light" size="sm" isIconOnly onPress={() => onDelete(node.id)} className="min-w-6 h-6 text-destructive/70"><Trash2 size={12} /></Button>
      )}
    </div>
  )
}
