...
function ManualBadge({ state }: { state: WikiManualState }) {
  ...
  return (
    <span
      className={`inline-flex items-center gap-1 rounded px-1.5 py-0.5 text-[9px] font-medium uppercase tracking-wide ${
        state === "locked"
...
function ReferencesSection({
  ...
  const uniqueRefs = [
    ...new Map(references.map((ref) => [referenceIdentity(ref), ref])).values(),
  ];
  ...
  return (
    <section className="wiki-references">
      <h3 className="wiki-references-title">References</h3>
...
export default function WikiDocumentView({
  ...
  const { t } = useLocale();
  const projectSource = useShellStore(
    (s) => s.projects.find((p) => p.id === projectId)?.source,
  );
  const projectRoot = projectSource?.localPath;
  const projectLocation =
    projectSource?.kind === "wsl" &&
    projectSource.distribution &&
    projectSource.wslPath
      ? {
          kind: "wsl" as const,
          distribution: projectSource.distribution,
          path: projectSource.wslPath,
        }
      : undefined;
  ...
  const previewDraft =
    draftPreviewActive && draftPreviewId ? draftsById[draftPreviewId] : null;
  const previewChange = useMemo(() => {
    if (!previewDraft || previewDraft.documentId !== document.id) return null;
    return (
      previewDraft.changes.find((c) => c.documentId === document.id) ?? null
    );
  ...
  if (!contentMd) {
    const isGenerating =
      snapshot?.status === "writing" && document.pipelineStage === "pending";
    return (
    ...
    >
      <header className="wiki-doc-header">
        <div className="flex flex-wrap items-center gap-2 mb-2">
          <span className="wiki-doc-type-badge">
        ...
        {previewChange && (
          <p className="mt-2 text-[11px] text-warning">
            Draft preview — showing proposed changes
          </p>
        )}
      </header>
      ...
      <div className="wiki-markdown relative">
        ...
        <GoalSelectionToolbar
          documentId={document.id}
          contentMd={contentMd}
          containerClass="wiki-markdown"
        />
      </div>
