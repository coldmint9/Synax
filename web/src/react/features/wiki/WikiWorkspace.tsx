...
import {
  useWikiGenerationEvents,
  type WikiGenPhase,
} from "../../../hooks/useWikiGenerationEvents";
...
import {
  resolveWikiWritingProgressCounts,
  resolveGeneratingDocumentId,
} from "./wikiWritingProgressCounts";
import PlanView from "./PlanView";
import PlanListView from "./PlanListView";
import { AgentDock } from "../agent-workspace/dock/AgentDock";
  ...
  return (
    <div className="flex h-full min-h-0 overflow-hidden">
      <aside
        ...
        className="flex shrink-0 flex-col wiki-panel"
      >
      ...
      <div onMouseDown={onMouseDown} className="wiki-separator shrink-0" />
...
export default function WikiWorkspace({ projectId }: { projectId: string }) {
  ...
  const handleGenerationConflict = useCallback(
    async (err: WikiGenerationConflictError) => {
      await loadProjectSnapshot(projectId);
      const phase: WikiGenPhase =
        err.generationStatus === "writing"
          ? "writing"
          : err.generationStatus === "reinitializing"
            ? "refreshing"
            : "refreshing";
  ...
  const handleStartGeneration = useCallback(async () => {
    ...
    clearForRegeneration,
    gen,
    handleGenerationConflict,
  ]);

  const refreshing =
    refreshTask.phase !== "idle" &&
    refreshTask.phase !== "completed" &&
    refreshTask.phase !== "failed";
  ...
  const writingProgress = resolveWikiWritingProgressCounts(
    documents,
    gen.progress,
  );
  const { done: writtenDocCount, total: writableDocTotal } = writingProgress;
  const isActivelyWriting =
    snapshot?.status === "writing" || (gen.active && gen.phase === "writing");
  const showIncompleteBanner =
    (snapshot?.status === "failed" || snapshot?.status === "partial") &&
    documents.length > 0 &&
    !isActivelyWriting;
  ...
  useEffect(() => {
    if (refreshTask.phase === "completed") {
      void loadDrafts(projectId).then(() => {
        const { draftsSummary: summary, draftPanelOpen: alreadyOpen } =
  ...
  return (
    <div className="flex h-full min-h-0 overflow-hidden">
              ...
              {showIncompleteBanner && (
                ...
                >
                  <div className="flex items-center gap-1.5">
                    ...
                    >
                      {snapshot.status === "partial"
                        ? t("wikiGenerationPaused", {
                            done: writtenDocCount,
                            total: writableDocTotal,
                          })
                        : t("wikiGenerationIncomplete", {
                            done: writtenDocCount,
                            total: writableDocTotal,
                          })}
                    </span>
                  ...
                  {writableDocTotal > 0 && (
                    <WikiProgressBar
                      aria-label={t("wikiWritingProgress", {
                        done: writtenDocCount,
                        total: writableDocTotal,
                      })}
                      done={writtenDocCount}
                      ...
                      color={
                        snapshot.status === "partial" ? "warning" : "danger"
                      }
                    />
                  )}
                  <div className="flex items-center gap-1.5">
                    <button
                      ...
                      disabled={
                        continuing || reinitializing || isActivelyWriting
                      }
      ...
      {/* ── Resize handle ── */}
      <div onMouseDown={handleMouseDown} className="wiki-separator shrink-0" />
      ...
      <main className="relative flex min-w-0 flex-1 flex-col overflow-hidden">
        ...
        >
          {selectedDoc ? (
            <div
              ref={scrollRef}
              className="min-h-0 flex-1 overflow-y-auto px-6 pb-16 pt-14 sm:px-10"
            >
      ...
      {showReinitConfirm && (
        <DialogOverlay>
          <div
            className="dialog-content w-full max-w-sm rounded-2xl border border-border/40 bg-card p-6 shadow-xl"
            role="dialog"
            aria-modal="true"
            aria-label={t("wikiReinitConfirmTitle")}
          >
            <h3 className="text-sm font-semibold text-foreground/90">
              {t("wikiReinitConfirmTitle")}
            </h3>
