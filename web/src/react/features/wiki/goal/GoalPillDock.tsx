...
export function GoalPillDock({ projectId }: Props) {
  ...
  const media = useMediaDraft(projectId, (parts) =>
    useWikiStore.setState({ goalComposerContentParts: parts }),
  );
  ...
  const hasSession =
    Boolean(goalSession.sessionId) && goalSession.status !== "idle";
  ...
  const queueWhileGenerating =
    Boolean(goalSession.sessionId) &&
    (goalSession.status === "running" ||
      goalSession.status === "waiting_permission");

  const sessionDisplayTitle = resolveGoalSessionDisplayTitle(
    goalSession,
    t("goalWorking"),
  );
  const pendingPermissions = listPendingGoalPermissions(
    goalSession.permissions,
  );
  ...
  const handleReplyPermission = useCallback(
    (permissionId: string, reply: "once" | "always" | "reject") => {
      return replyGoalPermission(permissionId, reply);
    },
    [replyGoalPermission],
  );
  ...
  const composer = (
    <GoalComposerPill
      ...
      onSubmit={() => {
        ...
        void submitGoal(projectId).then(() => {
          if (!useWikiStore.getState().goalComposerContentParts?.length)
            media.clear();
        });
      }}
      onStop={stopGoal}
            ...
            <div className="goal-dock-stack flex w-full flex-col items-center">
              ...
              {goalSession.sessionId && (isCompose || isChat) && (
                <InputQueueStrip
                  items={queuedInputs}
                  onRemove={(itemId) =>
                    void removeQueuedInput(goalSession.sessionId!, itemId)
                  }
                  onForce={(itemId) =>
                    void forceQueuedInput(goalSession.sessionId!, itemId)
                  }
                />
              )}
