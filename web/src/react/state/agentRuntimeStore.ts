...
function createAgentRuntimeStore(
  projectId: string,
): StoreApi<AgentRuntimeState> {
  return createStore<AgentRuntimeState>((set, get) => ({
    ...
    refreshRuns: async (sessionId) => {
      try {
        ...
        set({ runs: items });
        const active =
          items.find((r) => r.status === "running") ?? items[items.length - 1];
...
export function getAgentRuntimeStore(
  projectId: string,
): StoreApi<AgentRuntimeState> {
