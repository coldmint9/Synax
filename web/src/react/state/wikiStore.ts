...
import {
  ...
  type WikiPlan,
  type WikiPlanNode,
  type WikiPlanWithSummary,
  type PlanNodeDraft,
  type PlanStreamEvent,
  type PlanExecuteEvent,
  type WikiPlanNodeArtifact,
  type GoalAnchor,
} from "../../lib/api/goal";
import {
  agentRuntimeApi,
...
import {
  ...
  DEFAULT_GOAL_PERMISSION_TIER,
  type GoalPermissionTier,
  type GoalWikiAttachMode,
} from "../features/wiki/goal/goalAttachTypes";
import {
...
export interface WikiState {
  ...
  handleRefreshEvent: (
    type: string,
    data: { taskId: string; message: string; meta?: Record<string, unknown> },
  ) => void;
  ...
  updatePlanNode: (
    nodeId: string,
    updates: Partial<
      Pick<WikiPlanNode, "title" | "description" | "expectedFiles">
    >,
  ) => Promise<void>;
  ...
  setGoalPermissionTier: (tier: GoalPermissionTier) => Promise<void>;
...
function documentsTreeMetadataChanged(
  prev: WikiDocument[],
  next: WikiDocument[],
): boolean {
...
export const useWikiStore = create<WikiState>((set, get) => ({
  ...
  setSnapshotLoading: () => {
    set((s) => ({
      ...s,
      loading: { ...s.loading, snapshot: true },
      error: null,
    }));
  },
  ...
  loadProjectSnapshot: async (projectId: string) => {
    ...
    try {
      ...
      set((s) => ({
        ...
        error:
          err instanceof Error ? err.message : "Failed to load wiki snapshot",
      }));
    }
  },
  ...
  applySnapshotTree: (tree: WikiSnapshotTree) => {
    ...
    const docsChanged = documentsTreeMetadataChanged(
      prev.documents,
      tree.documents,
    );
    ...
    const snapshotChanged =
      ...
      prev.snapshot.documentIds.length !==
        (tree.snapshot?.documentIds.length ?? 0);

    const draftsChanged =
      prev.draftsSummary.ready !== (tree.draftsSummary?.ready ?? 0) ||
      prev.draftsSummary.generating !== (tree.draftsSummary?.generating ?? 0);

    const isSelectable = (d: { id: string; isSection?: boolean }) =>
      !d.isSection;
    const firstDocId = tree.documents.find(isSelectable)?.id ?? null;
    const currentSelected = prev.selectedDocumentId;
    const savedDocId = !currentSelected
      ? (localStorage.getItem("wiki-selected-doc") ?? null)
      : null;
    const restoredId =
      savedDocId &&
      tree.documents.some((d) => d.id === savedDocId && isSelectable(d))
        ? savedDocId
        : null;
    const selectedDocumentId =
      currentSelected &&
      tree.documents.some((d) => d.id === currentSelected && isSelectable(d))
        ? currentSelected
    ...
    const patch: Partial<WikiState> = {
      loading: {
        snapshot: false,
        plans: prev.loading.plans,
        drafts: prev.loading.drafts,
      },
      error: null,
    };
    ...
    if (draftsChanged)
      patch.draftsSummary = tree.draftsSummary ?? { ready: 0, generating: 0 };
    if (selectedDocumentId !== prev.selectedDocumentId)
      patch.selectedDocumentId = selectedDocumentId;
  ...
  selectDocument: (documentId) => {
    ...
    if (documentId) {
      try {
        localStorage.setItem("wiki-selected-doc", documentId);
      } catch {}
    }
  },
  ...
  setSearchHighlightQuery: (query) =>
    set((state) => ({
      ...
      searchHighlightNonce: query
        ? state.searchHighlightNonce + 1
        : state.searchHighlightNonce,
    })),
  ...
  loadDrafts: async (projectId, status) => {
    ...
    try {
      ...
      set((s) => ({
        ...s,
        draftsById,
        draftsSummary: { ready, generating },
        loading: { ...s.loading, drafts: false },
      }));
    } catch {
  ...
  selectDraft: (draftId) => {
    ...
    set((s) => ({
      ...
      draftSelectedDocumentIds: {
        ...s.draftSelectedDocumentIds,
        [draftId]: documentIds,
      },
      selectedDocumentId: draft.documentId,
    }));
    try {
      localStorage.setItem("wiki-selected-doc", draft.documentId);
    } catch {}
  },

  backToDraftList: () =>
    set({ selectedDraftId: null, draftPanelLayer: "list" }),
  ...
  editDraftChange: (draftId, documentId, newContentMd) => {
    set((s) => ({
      draftEditedContentMd: {
        ...
        [draftId]: {
          ...(s.draftEditedContentMd[draftId] ?? {}),
          [documentId]: newContentMd,
        },
      },
    }));
  },
  ...
  applyDraft: async (draftId, documentIds) => {
    ...
    try {
      if (edits && Object.keys(edits).length > 0) {
        const changes = Object.entries(edits).map(
          ([documentId, newContentMd]) => ({ documentId, newContentMd }),
        );
      ...
      set((s) => {
        ...
        return {
          ...
          draftsSummary: {
            ...s.draftsSummary,
            ready: Math.max(0, s.draftsSummary.ready - 1),
          },
        };
      });
    } catch {
    ...
    try {
      ...
      set((s) => {
        ...
        return {
          ...
          draftSelectedDocumentIds: restSelected,
          selectedDraftId:
            s.selectedDraftId === draftId ? null : s.selectedDraftId,
          draftPanelLayer:
            s.selectedDraftId === draftId ? "list" : s.draftPanelLayer,
          draftsSummary: {
            ...s.draftsSummary,
            ready: Math.max(0, s.draftsSummary.ready - 1),
          },
  ...
  loadPlans: async (projectId: string) => {
    ...
    try {
      ...
      if (selectedPlanId) {
        ...
        const active =
          plans.find(
            (p) => p.status !== "completed" && p.status !== "discarded",
          ) ?? null;
        set((s) => ({
          ...s,
          plans,
          activePlan: active,
          loading: { ...s.loading, plans: false },
        }));
      }
    } catch {
  ...
  confirmPlan: async (projectId: string, planId: string) => {
    ...
    set((s) => ({
      activePlan: s.activePlan
        ? { ...s.activePlan, status: "executing" as const }
        : null,
    }));
    ...
    goalApi.streamGeneratePlan(
      ...
      (event) => {
        ...
        switch (event.type) {
          ...
          case "phase":
            set({
              planGeneration: { ...s.planGeneration, phase: event.phase },
            });
            break;
          case "tool_call":
            set({
          ...
          case "message_delta":
            set({
              planGeneration: {
                ...s.planGeneration,
                streamingText: s.planGeneration.streamingText + event.delta,
              },
            });
            break;
          case "node_submitted":
            set({
              planGeneration: {
                ...s.planGeneration,
                previewNodes: [...s.planGeneration.previewNodes, event.node],
              },
            });
            break;
          case "completed":
            get()
              ...
              .then(() => {
                ...
                set({
                  planGeneration: { ...get().planGeneration, status: "idle" },
                });
                toast.push({
                  type: "success",
  ...
  setGoalComposerWikiAttachMode: (mode) =>
    set((s) => ({
  ...
  setGoalComposerReasoningEffort: (effort: ReasoningEffort) =>
    set({ goalComposerReasoningEffort: effort }),
  setGoalPermissionTier: async (tier) => {
    ...
    if (get().goalSession.sessionId === sessionId)
  ...
  submitGoal: async (projectId) => {
    ...
    const contentParts: RuntimeContentPart[] | undefined = s
      .goalComposerContentParts?.length
      ? [
          ...(content ? [{ type: "text" as const, text: content }] : []),
          ...s.goalComposerContentParts,
        ]
      : undefined;
    ...
    const clearAcceptedDraft = () => {
      ...
      const current = get();
      if (
        current.goalSession.sessionId !== targetSessionId ||
        useShellStore.getState().currentProjectId !== sourceProjectId
      )
        return;
      if (
        current.goalComposerContent === s.goalComposerContent &&
        JSON.stringify(current.goalComposerContentParts ?? []) ===
          JSON.stringify(s.goalComposerContentParts ?? [])
      ) {
        set({ goalComposerContent: "", goalComposerContentParts: [] });
      }
    };

    let targetSessionId = s.goalSession.sessionId;
    const sourceProjectId = useShellStore.getState().currentProjectId;
    const isFollowUp =
      Boolean(s.goalSession.sessionId) &&
    ...
    const model = formatTurnModel(
      s.goalComposerProviderId,
      s.goalComposerModelId,
    );
    ...
    try {
      if (isFollowUp && s.goalSession.sessionId) {
        ...
        if (shouldQueue) {
          await useAgentSessionStore
            .getState()
            .enqueueSessionInput(s.goalSession.sessionId, {
              message: content,
              contentParts,
              model,
            });
        ...
        set({
          ...
          goalDockState:
            s.goalDockState === "expanded" ? "expanded" : "working",
        });
        await streamGoalAgentTurn(
          ...
          (chunk) => {
            if (
              ...
              clearAcceptedDraft();
            set((state) =>
              state.goalSession.sessionId !== targetSessionId
                ? state
                : {
        ...
        if (get().goalSession.sessionId !== targetSessionId) return;
      ...
      const { prompt, wikiContext } = await goalApi.buildSessionPrompt(
        projectId,
        {
          mode: "direct",
          ...
          wikiAttachMode,
          documentId:
            wikiAttachMode === "manual" ? s.goalComposerDocumentId : null,
          documentTitle:
            wikiAttachMode === "manual" && s.goalComposerDocumentId
              ? (s.documents.find((d) => d.id === s.goalComposerDocumentId)
                  ?.title ?? null)
              : null,
          anchorJson:
            wikiAttachMode === "manual" ? s.goalComposerAnchorJson : null,
        },
      );
      ...
      const payload = await agentRuntimeApi.createSession({
        ...
        prompt,
        skillIds:
          s.goalComposerSkillIds.length > 0
            ? s.goalComposerSkillIds
            : undefined,
      ...
      if (
        get().goalSession.sessionId === s.goalSession.sessionId &&
        useShellStore.getState().currentProjectId === sourceProjectId
      )
        set({
          goalSession: {
  ...
  replyGoalPermission: async (permissionId, reply) => {
    ...
    set((s) =>
      s.goalSession.sessionId !== sessionId
        ? s
        : {
            goalSession: {
              ...s.goalSession,
              permissions: s.goalSession.permissions.map((p) =>
    ...
    const abort = goalApi.streamPlanExecution(planId, (event) => {
      if (event.type === "plan_status") {
        set((s) => ({
          activePlan: s.activePlan
            ? {
                ...s.activePlan,
                status: event.status as typeof s.activePlan.status,
              }
            : null,
        }));
      }
      ...
      if (event.type === "node_status") {
        set((s) => ({
          ...
          activePlanNodes: s.activePlanNodes.map((n) =>
            n.id === event.nodeId
              ? { ...n, status: event.status as WikiPlanNode["status"] }
              : n,
          ),
        }));
      }
      if (event.type === "plan_completed") {
        set((s) => ({
          planExecution: { ...s.planExecution, status: "completed" },
        }));
        if (get().activePlan)
          void get().loadActivePlan(get().activePlan!.projectId);
      }
